...
//
// **関数のタイムアウトより十分短くする。** ここで待ち続けると、打ち切るのは
// プラットフォームになり、`timings` も理由も残らない（main.go の deadlineMargin と
// 同じ考え方）。ビルドが終わったあとの残り時間で SSM 1 往復と PUT 2 本を行う。
//
// **秒数を書き写さない**（#165）。宣言（terraform/build-function.tf の
// `build_function_timeout_seconds`）は 10 → 25 → 30 → 45 秒と動いており、
// 写した側だけが取り残された前例がある。ここが要求するのは
// 「**ビルドと圧縮のあとに残る時間より短い**」ことだけで、その残り時間は
// タイムアウトからビルド実測（21〜24 秒）を引いた分である。15 秒はその内側にある。
