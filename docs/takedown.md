...
### アカウントを消したいという相談は、退会へ案内します（#518 / 仕様 8.1）

**2026-09-16 から、利用者は自分で退会できます**（登録情報の「アカウント」→「退会について確かめる」）。
**退会は、この 4.5 と同じ順序（排他を取る → R2 から消す → 自分の排他を持つ行だけ確定する）を、コードへ移したものです**
（`src/withdrawal.ts`）。いまのアイコンと差し替え前の画像の両方を、30 日を待たずに消します。

- **アカウントの削除の希望には、まず退会を案内してください。** この端末手順では、アイコンしか消えません
- **退会した人のアイコンが R2 に残っていないことは、`bash scripts/withdrawal-status.sh <利用者の id>` が読み取りだけで確かめます**
  （最終行が `WITHDRAWAL_COMPLETE` になるまで打ちます。`docs/cleanup-worker.md`）
- **この端末手順が要るのは、退会できない利用者だけになりました。** 該当するのは次の 2 つです
  - **BAN された利用者**（ログインできないので退会の画面へ行けません。仕様 8.1 / #518 の constraints）
  - **管理者（`users.is_admin`）**（退会そのものを断ります。先に D1 で権限を外せば退会できます）
- **BAN と退会の順序に注意してください。** 先に退会されると、その行の `google_sub` は結び付かない値へ置き換わるので、
  **同じ人が新しい招待で戻ってきても、旧い行とは結び付けられません**（#518 の J11。意図した扱いです）
