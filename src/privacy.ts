 ...
 * | チャットの会話（30 日） | `src/chat-conversation.ts`（`chat_conversations`。本人だけが読める・本人が消せる・cron が 30 日で消す・退会の段3 で消える。#695 が同じ変更で追記した） |
...
<h3>利用者が登録・入力する情報</h3>
<ul>
  <li><strong>表示名</strong>（設定の画面で変更できます）</li>
  <li><strong>表示名の変更の履歴</strong>: 表示名が変わったときの、変える前と後の表示名と、変えた日時。設定の画面で変えた場合のほか、Google アカウントの名前に合わせて表示名が変わった場合も残します。通報への対応のために運営者が確かめるもので、公開しません。変更の履歴は書き換えず、追記だけで残します。Cloudflare のデータベース（D1）に保存します</li>
  <li><strong>自己紹介と外部リンク</strong>（設定の画面で決められます）: 自己紹介の文章と、外部のページへのリンク（3 本まで）。作者ページで誰でも見られます。Cloudflare のデータベース（D1）に保存します</li>
  <li><strong>アイコンの画像</strong>（設定の画面で決められます）: 選んでいただいた画像を、中央で正方形に切り抜いて ${AVATAR_OUTPUT_SIZE} ピクセル四方の WebP に作り直したもの。<strong>元のファイルは保存せず、撮影した場所や機種などの情報（Exif などのメタデータ）は作り直すときに取り除きます。</strong>作者ページ・作品の一覧・ヘッダで誰でも見られます。作り直しは Amazon Web Services（AWS）上の処理で行い、作り直した画像を Cloudflare のファイルの保存場所（R2）に保存します</li>
  ...
  <li><strong>メール配信の設定</strong>（設定の画面で変更できます）: 作品がフォークされたときのお知らせを受け取るかどうかと、受け取らない設定にした日時。公開しません。Cloudflare のデータベース（D1）に保存します</li>
  <li><strong>AI アプリとの接続</strong>（Claude などの AI アプリに、あなたの作品の読み取りや生成を許可した場合）: 接続したアプリの名前（アプリ自身が名乗った名前）・許可した後の戻り先・許可した範囲・接続した日時と、アプリへ発行した鍵（トークン）の記録。<strong>鍵そのものは保存せず、照合に使う値（ハッシュ値）だけを保存します。</strong>設定の「接続中のアプリ」で確かめ、解除できます。Cloudflare のキーと値の保存場所（KV）に保存します</li>
  ...
  <li><strong>生成の前のチャットの会話</strong>（生成画面で AI とチャットして指示文を練った場合）: あなたが書いた文と、AI が返した指示文の下書き。<strong>あなただけが見られます（公開しません）。</strong>続きからチャットできるように保存し、<strong>最後に使ってから ${CHAT_RETENTION_DAYS} 日で自動的に削除します。</strong>生成画面の「チャットの記録を消す」でいつでも削除できます。Cloudflare のデータベース（D1）に保存します</li>
  <li><strong>チャットのときに、いつも守ってほしいこと</strong>（設定の「チャット」に書いた場合）: あなたが書いた文。<strong>あなただけが見られます（公開しません）。</strong>チャットのたびに、いちばん最初に AI へ伝えるために保存します。<strong>チャットにだけ使い、生成そのものには渡しません。</strong>設定の「チャット」で書き換え・削除（空にする）ができます。Cloudflare のデータベース（D1）に保存します</li>
...
<ul>
  ...
  <li>生成の完了・失敗や、作品がフォークされたことを、メールでお知らせするため（作品がフォークされたことのお知らせは、設定の画面で受け取らない設定にできます。生成の完了・失敗のお知らせは、その設定にかかわらず送ります）</li>
...
<ul>
  ...
  <li><strong>生成の前のチャットの会話は、最後に使ってから ${CHAT_RETENTION_DAYS} 日で自動的に削除します</strong>（削除の処理は 5 分ごとに動きます）。生成画面の「チャットの記録を消す」を押すと、その時点で削除します。退会したときも削除します（下の「退会」）。<strong>チャットにかかった費用の記録（日時・処理した文字量・費用）は残します</strong>（チャットの 1 日の上限とサービス全体の費用の上限を管理するため。会話の本文は含みません）。</li>
  <li><strong>チャットのときに、いつも守ってほしいことは、期限を定めた自動の削除を行いません</strong>（設定として置き続けるものです）。設定の「チャット」で空にすると、その時点で削除します。退会したときも削除します（下の「退会」）。</li>
  <li>それ以外の情報は、期限を定めた自動の削除を行っておらず、本サービスの提供に必要なあいだ保存します。</li>
</ul>
<h3>退会</h3>
<p><strong>利用者はご自身で退会できます。</strong>ログインしたうえで、設定の「アカウント」から
...
<ul>
  ...
  <li>生成の前のチャットの会話（上の ${CHAT_RETENTION_DAYS} 日を待たずに削除します。チャットの費用の記録は残します）。</li>
  <li>チャットのときに、いつも守ってほしいこと。</li>
