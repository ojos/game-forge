...
const SONNET_4_6: Omit<GenerationModel, 'key' | 'effort'> = {
  ...
  // 回数の差が `effort` の効果に見える。**この構造では片方だけ広げようがない**
  // ——2 群は同じ `SONNET_4_6` を展開している。
  //
  // ## 16,000 から 33,000 へ引き上げた（2026-09-04 / #284）
  //
  // 旧値の根拠は「5.3 がソースを 30KB（おおよそ 1 万トークン）に制限している」だった。
  // **その換算（3.07 バイト/token）が誤りだった**——`output_tokens` には thinking が
  // 乗るため、ソースのバイト数を出力トークンで割ると分母を取り違える。R2 の
  // `source.go` 4 本での実測は **2.0 バイト/output token**（1.99〜2.07）である。
  //
  // 上限 64KB（`src/system-prompt.ts` の `MAX_SOURCE_BYTES`）に要るのは
  // **32,768 トークン**。33,000 はそれを満たし、かつ**時間予算の不等式から解いた
  // 上限 33,638 トークンの内側**にある（生成レート実測 119.8 tok/s。n=30 /
  // R²=0.993 / 最遅 111.2。`terraform/orchestrator.tf` の
  // `orchestrator_generation_seconds` 297 秒がこれに対応する）。
  //
  // **したがって帯は 32,768 〜 33,638 の 870 トークン（≒ 1.7KB 分）しかない。**
  // `max_tokens` での切断率は**未測定である。** 切断が増えるようなら、先に動かすのは
  // この値ではなく生成の秒数と timeout のほうである（帯の上端が動かないと
  // この値も上げられない）。**上限サイズ・この値・時間予算は連動する。**
  maxTokens: 33_000,
};
