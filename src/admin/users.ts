 ...
 * ══════════════════════════════════════════════════════════════════════════════
 *
 * **出す理由が無いものを出さない**（`src/admin/home.ts` が持っていた規律。
 * `src/html.ts` の「要らない値を運ぶと、全画面の外枠がそれを出せる場所になる」と
 * 同じ向き）。誰であるかは表示名と id で足り、**アドレスで引く必要がある場面は
 * 端末の SQL にしかない**（`docs/admin-host.md` の `is_admin` の立て方）。
 */
