...
→ **試行回数は #284 で 3 → 2 になりました**（上限を 64KB へ上げると 1 試行の生成が
91 → 297 秒になり、3 試行では Lambda の実行時間にも `STALE_AFTER_SECONDS` にも
収まらないためです。正本は `terraform/orchestrator.tf`）。**機械修正の巡回数（2）は
動かしていません。** 打ち消した「約 16 円」は 2026-08-28 の値で、**いまは 1 生成
¥22.41**（2026-09-04 / 本番の既定群 20 件の平均。直近 10 件は ¥26.75。仕様 4.2 の
実測注記。#296）。**費用ゼロで直せる失敗を費用ゼロのまま直す段の価値は、単価が
上がったぶん高くなっています。**

## 基盤のリトライは 0 である

5.2-7（`src/build-retry.ts` の `MAX_GENERATION_ATTEMPTS`）が既にビルド診断を織り込む
賢い再試行を最大 2 回持っている（#284 で 3 → 2）。**`MaximumRetryAttempts` の既定は 2 で、
掛け算にすると 1 回の送信から最大 6 回・約 134 円・日次枠 6 個が出る**
（6 ＝ 既定リトライ 2 ＝ 3 配信 × 2 試行、134 ＝ 6 × 実測 ¥22.41）。

> **数え直しの跡（#296）。** ここは長く「最大 9 回・約 144 円・日次枠 9 個」と書いて
> いました。**9 ＝ 3 配信 × 3 試行、144 ＝ 9 × 約 16 円**です。#284 で試行が 3 → 2 に
> なり、#296 で単価が約 16 円 → ¥22.41 になったので、**両方を掛け直して 6 回・
> ¥134.46（表記は ¥134）**としています。**回数が 3 分の 2 になったのに総額は 7% しか
> 下がりません**（144 → 134.46）。単価が 1.40 倍（16 → 22.41）になったためです。
> 正本は `scripts/check-orchestrator-retry.sh` で、
> そちらは #284 の時点で更新済みでした（この文書だけが取り残されていました）。
...
### 前提: **どのツリーから叩いているか**（#290）

**`scripts/deploy-orchestrator.sh` は、叩いた作業ツリーから束ね直す。** worktree を
分けて並行作業していると、**別ブランチのツリーから叩いた**ときに、そのブランチの束を
本番と比べることになる。

**2026-09-04 にこれを踏んだ。** `main` へ入れた変更を配るつもりで、別ブランチの
worktree から叩いた。そのブランチの束がたまたま本番と同じだったため——
...
——と出て、**緑のまま本番は据え置きだった。** 気づいたのは CI の `deploy` が
落ち続けたからで、**成功の文言だけを読んでいたら分からなかった。**

配る前に、いまどこに居るかを見ること。

```bash
pwd
git log --oneline -1     # 配りたいコミットであること
```

配ったあとは、**出た `CodeSha256` を読むこと。** 「載せ替えます: <古い> -> <新しい>」が
出ていなければ、何も起きていない。
...
### 契約に合わない呼び出しでも、行は閉じる（#242）

**2026-09-01、コールバックを 1 通も送らずに落ちた。** 登録簿のずれでペイロードが契約に
合わなくなり（#241）、**作品行は `generation_state='pending'` のまま残った。** 作者の画面は
**15 分**のあいだ「生成中です／通常 1〜2 分かかります」を出し続け（`STALE_AFTER_SECONDS`）、
**利用者からは「遅い」としか見えなかった。**

**いまは、宛先（`gameId` / `jobToken`）が読めるなら閉じる。**

```
claim（pending → running）→ finishWithError('internal')（running → failed）
```

- **`finish` は `running` の行にしか効かない**ので、先に `claim` で握る
- **握れなければ何もしない**——別の呼び出しが進めている行を、こちらが閉じてはいけない
- **`buildPathFailed` は false。** ビルド経路を 1 度も触っていないので 3.8 の degrade を焚かない
- **元の拒否は隠さない。** ここでの失敗は飲み込み、`OrchestratorPayloadRejected` を投げ続ける
  ——**DLQ と CloudWatch に残る「契約違反」が、この付け足しで消えてはいけない**

**これは #241 の代わりではない。** 拒否そのものを防ぐのは配備の順序で、こちらは
**それでも起きたときに、黙って待たせない**ための経路である。
...
費用は **2026-09-04 / 本番の既定群 20 件の平均**（直近 10 件は ¥26.75。仕様 4.2 の
実測注記）。**#284 でソース上限を 64KB へ上げたので上がる向きに動く**——大きな作品が
出るお題を選ぶと、この 1 回で ¥75 まで行きうる。**確認のお題は小さいものにする。**
