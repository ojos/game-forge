 ...
 *
 * **引数の形の誤りは分類名にならない**（#707 の Copilot の指摘を受けて約束を明記した）。道具の入力の定義（`z.strictObject`）は AI が
 * 道具を正しく呼ぶための手がかりなので厳しいまま残し、必須の引数が無い・型が違う・定義に無い引数がある要求は、**道具の中身に届く前に
 * SDK が断る**。実測（2.0.0。旧版・2026-07-28 版とも）では、JSON-RPC の invalid params ではなく、HTTP 200 の中の道具の失敗
 * （`isError: true`。本文は `Input validation error: Invalid arguments for tool …` の素のテキスト）になる。余分なキーは落とさずに断る
 * （`Unrecognized key`）。既存の口と同じ分類名で返すのは、形の正しい引数の中身の検証（空・長すぎる・自作でない・枠切れ・進行中など）だけ。
 * `test/mcp-server.test.ts` が固定する。
 *
