...
# ── トークンの宣言の置き場所（冒頭の規約 4）──────────────────────────────────
#
# **画面の区画でトークンを定義させない。** 予約区画へ `--gf-*` が漏れると、値の正本が
# 2 か所になり、次のレーンはどちらを直せばよいか分からなくなる。**使う側（`var(--gf-*)`）は
# 数えない**——行頭が `--gf-` で始まる宣言だけを見る。
/^[[:space:]]*--gf-[a-z0-9-]+:/ {
  if (current != "tokens" && current != "shell") {
    problem(css ":" NR " トークンの宣言は @section tokens（段ごとの差し替えは @section shell）" \
      "にしか置けません（いまは @section " (current == "" ? "（区画の外）" : current) "）。")
  }
}
...
END {
  ...
  # **段の数そのものを見る。** 覆われているかだけを見ると、境界を 1 つ消して 2 段に
  # しても（390 と 768/1280 が残った 2 段を覆うので）通ってしまう。**3 段は 2.3.9 の
  # 決定**なので、増減するならあちらとこの行を同じコミットで直させる
  # （Copilot の指摘。2026-09-12）。
  if (tier_count != 2) {
    problem(css " の幅の @media の境界が " tier_count " 件です。2.3.9 は 3 段（境界 2 件）と" \
      "決めています。段を増減するなら、2.3.9 とこの検査を同じコミットで直してください。")
  }
