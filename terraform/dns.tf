 ...
 * DNS の宣言（確定17）。
 *
 * # #775 で使われなくなった（削除は別 issue）
 *
 * **ojos.jp ごと Cloudflare DNS へ移し、game-forge.ojos.jp のレコードもそのゾーンへ吸収した**
 * （terraform/dns-ojos-jp.tf。確定17 の改訂）。段 C2 で委譲の NS を外した後、この Route 53 の
 * ゾーンに問い合わせは来ない。**消さずに残しているのは、キャッシュに残った委譲を辿ってくる
 * リゾルバへ同じ答えを返し続けるため**である。
 *
 * **このファイルのレコードは、Cloudflare 側の値の供給元でもある**（dns-ojos-jp.tf が参照する）。
 * 値や注記を変えるときはここを直せば両方に効く。**Route 53 のゾーンを消す issue では、先に値と
 * 注記を dns-ojos-jp.tf へ移すこと。**
 *
 * **`aws_route53_zone` の `comment` を書き換えないこと。** 属性の変更がゾーンの作り直しになり、
 * ネームサーバが変わる。
 *
 * 以下は Route 53 へ委譲していた時点の記述として残す。
 *
