 ...
 * | 改造の通知（5.5 / #36） | **フォークの公開 1 件につき 1 通** | 改造は生成の一種なので**上の 34 通の内数** |
 *
 * **上限に近づく形は 1 つだけある**——1 通の生成が複数人へ通知を生む場合（5.5 の系統が
 * 深いときに祖先へ配るなど）である。**#36 はその形を選ばなかった**——送るのは
 * **直近の親の作者 1 人だけ**で（`src/mail/fork-notice.ts`）、系統をさかのぼらない。
 * したがって見積もりは上の表のまま変わらない。**祖先へ配る形を足すときは、そこで
 * 日次の見積もりを立て直すこと。** 本モジュールは 1 回の呼び出しで 1 通しか送らない。
 *
 * **契機が「生成」ではなく「公開」であることも、上限を押し下げる側に効く。**
 * 生成されたフォークがすべて公開されるわけではない（5.4）。
 *
