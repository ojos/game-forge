...
#   1 = BUILD_HEADROOM_NEAR     接近している（または超過した呼び出しがある）
#   2 = BUILD_HEADROOM_UNKNOWN  判定できない（未認証・ログが無い・呼び出しが 0 件）
#
...
[build-time] 閾値の導出（AWS へは触れていません）
  ...
  超過            ${TIMEOUT_SECONDS} 秒以上（打ち切られている。1 件でも不合格）
...
#
# 出力は 1 行 1 呼び出しの TSV: <JST 日付> <所要ミリ秒> <コールドか>
SAMPLES="$(jq -r '.events[] | [(.timestamp | tostring), .message] | @tsv' <<<"$EVENTS" \
  | awk -F '\t' -v off="$REPORT_WINDOW_JST_OFFSET_SECONDS" '
      {
        ts = $1; msg = $2
        if (msg ~ /Task timed out/) { timedout++; next }
        if (msg !~ /REPORT RequestId/) next
        if (match(msg, /Duration: [0-9.]+ ms/) == 0) next
        dur = substr(msg, RSTART + 10, RLENGTH - 13) + 0
        cold = (msg ~ /Init Duration:/) ? 1 : 0
        # 日付は呼び出しの記録時刻から作る。JST の境界は report-window.sh の定数。
        day = int((ts / 1000 + off) / 86400)
        printf "%d\t%.0f\t%d\n", day, dur, cold
      }
      END { printf "#timedout\t%d\t0\n", timedout + 0 }
    ')"

TIMED_OUT_LINES="$(awk -F '\t' '$1 == "#timedout" { print $2 }' <<<"$SAMPLES")"
SAMPLES="$(awk -F '\t' '$1 != "#timedout"' <<<"$SAMPLES")"
SAMPLE_COUNT="$(printf '%s' "$SAMPLES" | grep -c . || true)"

if [[ "$SAMPLE_COUNT" -eq 0 ]]; then
  echo "[build-time] ${LOG_GROUP}"
...
STATS="$(printf '%s\n' "$SAMPLES" | awk -F '\t' -v near="$NEAR_MS" -v ceil="$TIMEOUT_MS" '
...
TOTAL="$(printf '%s\n' "$SAMPLES" | awk -F '\t' -v near="$NEAR_MS" -v ceil="$TIMEOUT_MS" '
  { n++; if ($2 > max) max = $2; if ($2 >= near) nearc++; if ($2 >= ceil) overc++
    if ($3 == 1) cold++; a[n] = $2 }
  END {
...
# ── 判定 ────────────────────────────────────────────────────────────────────
#
# **超過が 1 件でもあれば落とす。** 超過は予告ではなく発生であり、そのとき利用者は
# すでに踏んでいる。接近は割合で見る（1 回の外れ値で鳴らすと、鳴っても見なくなる）。
...
if (( T_OVER > 0 )); then
  ...
  EXIT_CODE=1
  REASON="天井へ達した呼び出しが ${T_OVER} 件あります（打ち切られています）。"
...
emit_table() {
  echo "[build-time] ${LOG_GROUP}（CloudWatch の REPORT。読み取りのみ・新しい計測は足していません）"
  report_window_describe
  echo
  {
    printf '日付\t呼び出し\t中央値(秒)\tp95(秒)\t最大(秒)\t接近\t超過\tコールド\n'
  ...
  if [[ -n "$TIMED_OUT_LINES" && "$TIMED_OUT_LINES" != "0" ]]; then
    printf '  - "Task timed out" のログが %s 行あります。\n' "$TIMED_OUT_LINES"
  fi
  echo
  echo "判定: ${REASON}"
}
