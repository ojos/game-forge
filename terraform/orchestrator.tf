...
locals {
   ...
   * タイムアウト（秒）。
   *
   * **5.2-7 の最大 3 試行が収まる幅にする。** 1 試行は生成 91 秒＋ビルド最大 30 秒で、
   * 4.2 の機械修正でビルドがもう一度走ることもある。3 試行の最悪で約 450 秒。
   * 600 秒はその上に余裕を置いた値である。
   *
   * **`src/work-page.ts` の `STALE_AFTER_SECONDS`（900 秒）より短い。** 画面が
   * 「中断した可能性」と言い始める前に、関数は必ず生きているか死んでいるかの
   * どちらかになっている。逆順にすると、まだ走っている生成を画面が中断と呼ぶ。
   */
