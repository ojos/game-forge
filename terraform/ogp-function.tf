...
locals {
   ...
   *
   * **同じ作品が 7.1〜16.9 秒でぶれる**（`ff7d397e` を 4 回測った実測）。最悪実測
   * 16,907 ms のとき、**20,000 ms に残る余裕は 3,093 ms（天井の 15%）** しかなく、
   * **同じ作品のぶれ（最大 9,843 ms）1 回分に足りない。**
   *
   * **割合は天井を基準にする**（`scripts/build-time-report.sh` の `NEAR_RATIO` と同じ
   * 数え方。天井に対して何割残っているか）。
   *
   ...
   *
   * **30,000 ms なら余裕は 13,093 ms（天井の 44%）になる。** 上の
   ...
   * 予約同時実行数。
   *
   * **2。** 公開は 1 作品につき 1 回きりの操作で、日次の生成が全体で約 29.7 回
   * （確定25。#284 で 34 回から引き直した）である以上、**同時に 3 枚撮る場面は事実上
   * 無い。** **母数が減ったので、この理由は #284 の前より強い。** 撮影は公開時の別操作で
   * 所要時間も変わっていないため、生成が長くなったこと（91 → 297 秒）はここに効かない。 予約は上限であると
   ...
   *
   * 0 にしている `terraform/orchestrator.tf` の理由は「5.2-7 の試行と掛け算になり、
   * 1 回の送信から最大 6 回・約 134 円が出る」ことだった（#284 で試行が 3 → 2 になり、
   * 9 回・約 144 円から下がった）。**こちらに掛け算の相手が無い。**
