#!/usr/bin/env node
// source-quality-backfill.mjs — 生成物の質の指標を既存作品へ埋め戻す（#605 / マイグレーション 0046）
//
// **入口は `scripts/source-quality-backfill.sh` である**（本番の資格情報を環境へ移してから、ここを呼ぶ）。
//
// ## なぜ埋め戻しが要るのか
//
// **#597 の基準線は、使い捨ての Go の道具（`go/parser`）で 1 回だけ測った値である。**
// 埋め戻して測り直さないと、**以後の前後比較が違う物差しで行われる。** 実装を変えたときに
// 「質が変わった」のか「測り方が変わった」のかを区別できなくなる。
//
// ## 何をするか
//
// 1. **対象を D1 から読む**——`games.source_key`（NULL を除く）と `game_revisions.source_key` の和集合のうち、
//    `source_quality_metrics` の行が無いか `rule_version` が古いもの。**綴りは
//    `src/source-quality-metrics.ts` の `SOURCE_QUALITY_TARGETS_SQL` をそのまま使う**（写さない）。
// 2. 既定（dry-run）は**件数と一覧を出して終わる。1 行も書かない。**
// 3. `--apply` のときだけ、対象ごとに **R2 からソースを読み**、`src/source-quality.ts` の
//    `measureSourceQuality` で測って `UPSERT_SOURCE_QUALITY_SQL` で書く（新しい版だけが上書きする
//    ＝何度流しても壊れない）。**字句解析が通らないソースは書かず、NG として残す**——測れなかったことを
//    0 として書くと、悪い生成と区別できなくなる。
// 4. 書いたあと、**対象を数え直す**（報告された件数を信じない。`scripts/input-keys-backfill.mjs` と同じ規律）。
//
// ## 値を実行時に読む（#380 の教訓）
//
// バケット名も対象の一覧も、**この道具が動くときに読む。** 事前に埋めた値は古くなる。
//
...
const TAG = '[source-quality-backfill]';
...
const work = mkdtempSync(path.join(tmpdir(), 'source-quality-backfill-'));
process.on('exit', () => rmSync(work, { recursive: true, force: true }));
const bundled = path.join(work, 'source-quality.mjs');
const entry = [
  `export { SOURCE_QUALITY_RULE_VERSION, measureSourceQuality } from ${JSON.stringify(path.join(ROOT, 'src', 'source-quality.ts'))};`,
  `export { SOURCE_QUALITY_TARGETS_SQL, UPSERT_SOURCE_QUALITY_SQL } from ${JSON.stringify(path.join(ROOT, 'src', 'source-quality-metrics.ts'))};`,
  `export { isStoredSourceKey } from ${JSON.stringify(path.join(ROOT, 'src', 'source-input-keys.ts'))};`,
].join('\n');
...
const {
  ...
  SOURCE_QUALITY_TARGETS_SQL,
  UPSERT_SOURCE_QUALITY_SQL,
  isStoredSourceKey,
...
function d1(sql) {
  ...
  if (!out.ok) {
    ...
    if (text.includes('no such table')) {
      lines.push('表がありません。マイグレーション（0046）が未適用の可能性があります。');
    }
    abort(lines);
  }
...
function readTargets() {
  const rows = d1(bindLiterals(SOURCE_QUALITY_TARGETS_SQL, [SOURCE_QUALITY_RULE_VERSION]));
  return rows.map((row) => {
...
console.log(`${TAG} 対象: ${where} / 規則の版: ${SOURCE_QUALITY_RULE_VERSION} / ${apply ? '書き込む（--apply）' : 'dry-run（書き込まない）'}`);
...
if (!apply) {
  ...
  if (invalid.length > 0) {
    console.log('SOURCE_QUALITY_BACKFILL_INCOMPLETE');
    process.exit(1);
  }
  console.log('SOURCE_QUALITY_BACKFILL_PASS');
  process.exit(0);
}
...
for (const key of targets) {
  ...
  const measured = measureSourceQuality(source);
  if (!measured.ok) {
    // **測れなかったことを 0 として書かない**（`src/source-quality.ts`）。行が無いことが
    // 「測れていない」を表す。残ったものは最後に INCOMPLETE として報告する。
    console.error(`${TAG} NG ${key} — 字句解析が通りません（${measured.reason}）`);
    unreadable.push(key);
    continue;
  }
  const m = measured.metrics;
  console.log(
    `${TAG} OK ${key} win=${m.hasWinText ? 1 : 0} lose=${m.hasLoseText ? 1 : 0} colors=${m.colorCount} sprites=${m.spriteCount} states=${m.stateCount}`,
  );
  statements.push(
    bindLiterals(UPSERT_SOURCE_QUALITY_SQL, [
      key,
      m.hasWinText ? 1 : 0,
      m.hasLoseText ? 1 : 0,
      m.colorCount,
