...
| 画面 | `src/admin/home.ts`（**いまは空の 1 枚**。審査キュー・BAN は M10-3 以降） |
...
- `<h1>管理</h1>` が出る（`is_admin = 1` が効いている）
...
**この PR で乗せなかった理由は、所有範囲である**（`scripts/` はこのレーンの所有外）。
**代わりに手で 1 度測った**（下記「実施の記録」）。**M10-3 が審査キューの表を置くとき、
そこが乗せる契機である**——表は狭い端末でいちばん崩れやすく、そのとき初めて
「測る対象」が生まれる。
