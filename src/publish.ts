 ...
 *
 * ## 公開はやめられる（#637 / 確定35。2026-09-17）
 *
 * **`published` → `draft` へ戻す口がある**（`src/work-page.ts` の `POST /api/works/unpublish`）。
 * したがって**この経路は同じ作品を 2 度以上通る**——`publishGame` は `published_at` を初回だけ書き
 * （`coalesce`）、タグはフォームが運んだ値で置き換える（`src/games.ts` の #637 注記）。
 * **撮影と改造の通知は、いまも「この呼び出しが実際に公開したとき」だけ起こす**——再公開でも
 * `firstTime` は真になるが、撮影は公開をやめたときに `ogp_state` を戻してあるので意図どおり撮り直し、
 * 通知は `fork_notices` の主キーが 2 通目を止める（`src/mail/fork-notice.ts` が「公開の経路を通らない
 * 2 通目」を想定して持っている段である）。
 *
 * > **#26 の時点の本文は「公開の取り消しはこの issue の範囲に無い」で、理由を「戻せることにすると、
 * > 共有された URL が黙って死ぬ」と書いていた。** #636 がこれを覆した——**`removed`（#35 の取り下げ）でも
 * > 共有 URL は同じように死ぬ**ので、`draft` を禁じる理由になっていなかった。`removed` は運営の措置（8.4）と
 * > 退会（確定34）の専用状態になった。
 */
 ...
 *
 * **公開の口（`POST /api/publish`）と、エディットページのまとめて保存する口（#664。`src/work-save.ts`）と、
 * 「あなたの作品」の一括の口（#666。`src/works-bulk.ts`）が同じ 1 本を通る。** 撮影と通知の起こし方を 2 か所に書くと、片方だけが「実際に公開したとき」の条件を外す。
