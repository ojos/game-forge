 ...
 * | `timeout` | `BuildTimedOut` | しない | 3.8 の degrade の対象。診断が無く、生成をやり直しても関数側の事情は変わらない |
 ...
 *
 * ## まだ実装していない段（4.2 の「リトライは 2 段構成にする」）
 *
 * 4.2 は #20 の範囲として「1. 機械修正（費用ゼロ）— 未使用 import の除去、不足 import の
 * 補完 / 2. LLM 再生成（費用が発生）」の 2 段を書いている。**本 issue が実装したのは
 * 2 だけである**（issue #20 の scope.in も 2 だけを挙げている）。使われない段を先に
 * 作らないので、1 のための継ぎ目もここには置かない。**挿入位置は
 * `src/generate.ts` のループの中、ビルドが `kind='build'` で落ちた直後**であり、
 * 費用ゼロの段なので台帳の行を作らず、日次クォータにも数えない（確定25）。
 */
