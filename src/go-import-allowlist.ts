 ...
 * ## 音は合成だけを許す（#286）
 *
 * `github.com/hajimehoshi/ebiten/v2/audio` を入れ、**デコーダ（`audio/vorbis` /
 * `audio/mp3` / `audio/wav`）は入れない。** 3 つはどれも「バイト列を渡すと音になる」
 * 口であり、**外部アセットを持ち込む経路そのもの**である。6.2 が IP 防御の要として
 * 「外部アセット禁止こそが最大の防御」と述べている根拠を直接崩すので、**音を許すことと
 * デコーダを許すことは別の判断**として扱う。
 *
 * **合成は 6.1 の「外部ファイルロードの全面禁止」に抵触しない。**
 * `Context.NewPlayerF32` が受け取るのは `io.Reader` で、生成コードは
 * `Read(p []byte) (int, error)` を持つ自前の型を渡す。**ファイルを 1 つも開かず、
 * 持ち込むアセットは 0 バイトである**（埋め込みフォントを許した理由と同じ形。
 * 禁じているのは実行時に外部から取ってくる経路であって、音という概念ではない）。
 * **#567 で、教える形はコードで計算した `[]byte` を `Context.NewPlayerF32FromBytes` へ
 * 渡す形に変わった**（理由は `src/system-prompt.ts` の #567 の節）。こちらも `io` /
 * `bytes` を import せずに書けるので、この一覧は変わらない。
 *
 * **どちらの `NewPlayer` を教えるかは、この一覧の責務ではない**（パッケージ単位なので
 * 許可の広さは変わらない）。ただし**「後から外すのは危険」は API の選択にも効く**——
 * 32bit float 版を教える判断とその理由はシステムプロンプト側（`src/system-prompt.ts`）
 * にある。
 *
 * **`io` は足さない。** メソッド 1 本を生やすだけなら `io` の型は要らず、終わりの無い
 * ストリームなら `io.EOF` も返さない（実測: `Read` を持つ自前の型だけで
 * `GOOS=js GOARCH=wasm` のビルドが通る）。**`io` を許すと `io.Reader` を取る API 全体が
 * 視野に入る**ので、要らないものは入れないという上の方針どおり足さない。
 *
