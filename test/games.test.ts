...
describe('プロンプトの宣言から題名を決める（#365）', () => {
  ...
  it('C1 制御文字と Unicode の行区切りも落とす（PR #385 のレビュー指摘）', () => {
    // **範囲を書き並べると C1 が落ちる。** #365 の初版は `[\u0000-\u001f\u007f]` と
    // 書いており、U+0085（NEL）がそのまま題名へ残った。表示名（5.9 /
    // `src/account.ts` の `FORBIDDEN_CHARACTER`）と同じ組で見る。
    expect(normalizeTitle('横\u0085スクロール')).toBe('横 スクロール');
    expect(normalizeTitle('横\u009fスクロール')).toBe('横 スクロール');
    // U+2028 行区切り / U+2029 段落区切り（`\p{Zl}` / `\p{Zp}`）。
    expect(normalizeTitle('横\u2028スクロール')).toBe('横 スクロール');
    expect(normalizeTitle('横\u2029スクロール')).toBe('横 スクロール');
    // 宣言の経路も同じ関数を通るので同じ結果になる。
    expect(draftTitleFromPrompt('タイトル: 横\u0085スクロール')).toBe('横 スクロール');
    // **絵文字の合成（U+200D ゼロ幅接合子）は落とさない**（`\p{Cf}` であり、
    // `src/account.ts` も丸ごとは弾いていない）。
