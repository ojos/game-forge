 ...
 * | 層 | 実体 | 捕まえるもの | 持ち主 |
 * |---|---|---|---|
 * | 1. アプリ層 | D1 の費用台帳＋月次 2 万円判定 | 主。80% 警告・残枠 UI・degrade | アプリ（#84） |
...
locals {
   ...
   *
   * prod 170 USD / dev 10 USD。**アプリ層の月次上限より先に発火してはいけない**
   * という制約から来ている（規則は「**円高側（130 円/ドル）へ振れても、層 1 の
   * 上限（円）を上回ること**」）。
   *
   *   170 USD × 130 円/ドル ＝ **¥22,100 > ¥20,000**（余裕 10.5%）
   *
   * ## 85 → 170 の理由（2026-09-04 / #284）
   *
   * **層 1 の上限が 1 万円から 2 万円になったので、同じ規則を当て直しただけである**
   * （新しい判断ではない）。85 USD のまま据え置くと **¥12,750（150 円/ドル）で
   * 層 3 が先に発火し、層 1 の 2 万円へ届く前に Bedrock が止まる。**
   *
   * **それは「遅い側の層が速い側を追い越す」ことを意味する。** 層 3 の動作は
   * **明示的 Deny の付与**で、**復旧は手動**である（4.3）。順序が反転したままだと、
   * 正常な利用が月の途中で止まり、人が Deny を外すまで戻らない。
   *
   * **余裕率は 85 USD のときと同じ 10.5% である**（85 × 130 ＝ ¥11,050 は 1 万円の
   * 110.5%、170 × 130 ＝ ¥22,100 は 2 万円の 110.5%）。**上限を動かしたらここも
   * 同じ比で動かす**——`src/quota.ts` の `MONTHLY_COST_LIMIT_JPY` が正本である。
   *
   * dev が別枠なのは、開発の実験を本番の枠に混ぜると 4.3 の判定基準が壊れるためである
   * （仕様 9.2 / 確定21）。dev は人が SSO の一時資格情報で手で叩く場所なので、
   * 予算は事故の上限としてのみ置く。**dev は層 1 の上限と連動しないので動かさない。**
   */
  bedrock_budget_prod_usd = "170"
 ...
 * ことによるずれは**早く発火する側**へ働き、為替バッファの向き（アプリ層の月次上限より
