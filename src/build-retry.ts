 ...
 *     材料の無い再生成は**1 生成 ¥22.41 と日次枠 1 回を賭けるだけ**である
 *     （2026-09-04 / 本番の既定群 20 件の平均。4.2 の実測注記）。
 *
 ...
 * 費用ゼロの段と **¥22.41 の段**が同じ引き金を共有することになる。
 *
 ...
 *    Sonnet 4.6 なら 33,000 トークン（実測の通常値 4,171 の約 7.9 倍）である。
 *    2 試行に回すと 1 リクエストで通常の約 16 倍を燃やす（**#284 で試行が 3 → 2 へ
 *    減ったのに倍率は上がっている**——`maxTokens` が 16,000 → 33,000 になったほうが
 *    効くためで、この分岐を引き金にしない理由はむしろ強くなった）。4.3 の「判断に迷う側は
...
/**
 * 1 リクエストで許す LLM 呼び出しの最大回数（5.2-7 の「初回＋1」）。
 *
 * **仕様書は再試行の回数（1）で、この定数は試行の総数（2）である。** 数え方が違う
 * ものを同じ数字にすると、どちらの意味で書かれた数なのかが読めなくなる。
 * 仕様書 5.2-7 の「最大1回」との一致は `test/generate.test.ts` が機械照合する
 * （shared-ai-rules 12 章）。
 *
 * ## 3 から 2 へ減らした（2026-09-04 / #284）
 *
 * **「試行回数（3）は動かさない。減らすと毎回の生成の成功率が下がる」という
 * `terraform/orchestrator.tf` の記述は、64KB 化で覆った。** 上限を 30KB → 64KB へ
 * 上げると 1 試行あたりの生成が 91 秒 → 297 秒になり、3 試行では Lambda の
 * 実行時間の上限（900 秒）にも `STALE_AFTER_SECONDS`（900 秒）にも収まらない。
 *
 *   2 × 297 ＋ (2 × 3 ＋ 1) × 25 ＋ 60 ＝ **829 ≤ 870**（余裕 41 秒）
 *
 * **「上限サイズを上げる」と「試行を 3 回持つ」は同時に成り立たない。** 前者を採った。
 * 4.2 の実測では初回コンパイル成功率が 5/6 で、**2 試行目までに落ちる分は
 * 大きくない**（費用ゼロの機械修正は各試行の中で最大 2 巡回るので、そちらは減らない）。
 *
 * **費用の側でも減る向きに効く。** 1 生成の実費は実測 ¥22.41 で、3 試行が 2 試行に
 * なることで 1 リクエストの最悪費用が約 67 円 → 約 45 円になる。
 */
