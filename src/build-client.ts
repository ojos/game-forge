 ...
 * **1 生成 ¥22.41 と日次枠 1 回**（2026-09-04 / 本番の既定群 20 件の平均。4.2 の
 * 実測注記）を、材料の無いまま賭けることになる。**やり直すのは
 ...
 * つき 1 回」として実装しており、**1 依頼で最大 9 回積めた**（**当時は 5.2-7 が 3 試行
 * だったので 3 ×（1 ＋ 2）＝ 9。#284 で 2 試行になったので、同じ後退をいま踏めば 6 回で
 * ある**）。**上限の単位が、上限を決めた根拠（オーケストレータの実行時間）の単位と
 * 違っていた**（{@link BuildTimeoutBudget}）。
 *
 ...
 *
 * **実測で裏づけた**（1.2.44。同日にビルド関数を 1 回直接叩いた）。合計 11.9 秒で、
 * 天井 20 秒に対して 41% の余裕がある。**compress の所要は 2,373 → 2,363 ms でほぼ
 * 動かなかった**——単一スレッドなので vCPU を増やしても効かない、という予告どおりである
 * （**動かないのは所要時間であって、圧縮率ではない**）。
 */
...
export interface BuildDependencies {
   ...
   * それだと 1 依頼あたりの呼び直しが最大 6 回
   * （`MAX_GENERATION_ATTEMPTS ×（1 ＋ MAX_MECHANICAL_FIX_PASSES）`＝ 2 × 3）になり、
   * オーケストレータの実行時間の見積もり（`terraform/orchestrator.tf`）が成り立たない。
   *
 ...
 * **依頼あたりでは呼び直しが最大 9 回積めた**（**当時は 3 試行なので 3 × 3 ＝ 9。
 * #284 以降は 2 × 3 ＝ 6 になる。式は同じで、掛かる試行回数だけが動く**）。 見積もりは 1 回ぶんしか見ておらず、
