/**
 * チャットの関数（#695 / M18-2。仕様 5.16「置き場所——エッジは Bedrock を呼べない」）。
 *
 ...
 * 実行体だけ**である（仕様 4.1）。
 *
 * **オーケストレータに相乗りさせなかった**（利用者の決定。5.16）。相乗りすると**チャットのコードを
 * 触るたびにオーケストレータの束が変わり、配り直すまで main の配備が全部止まる**
 * （`scripts/orchestrator-bundle-changed.sh` が起動する関門）。チャットは画面に近い機能で、
 * 生成の経路より直す頻度が高い。
 *
...
locals {
   ...
   * タイムアウト（秒）。
   *
   * **30 秒。** チャットの返答は出力 1,500 トークンが上限（`src/chat-payload.ts` の
   ...
   * 予約同時実行数。
   *
   * **2。** チャットは 1 人 60 秒 60 回の上限の内側で呼ばれ（`src/chat-paths.ts` の
...
data "aws_iam_policy_document" "chat" {
  ...
  /**
   * チャットの呼び出しと、8.2 の Guardrail の適用。
   *
   * **定義を `terraform/bedrock.tf` と共有する。** 動作の一覧をここへ書き写すと、
   * **Bedrock の API が増えた日に、生成だけが追随してチャットが置いていかれる**（あるいはその逆）。
