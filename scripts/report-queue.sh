#!/usr/bin/env bash
# report-queue.sh — 運営が見るべき作品を、本番の台帳から読む（8.4 / #40 / #366 / #394）
#
# ## なぜスクリプトなのか
#
# **仕様書 8.4 は運用画面を要求していない。** 求めているのはワンタップ通報・審査キューへの
# 投入・削除依頼フォーム・記録の 4 つで、画面はどこにも出てこない（#40 の intake /
# 2026-09-03）。
#
...
#
# ── 出すのは 2 種類である（#366 で 1 つ増え、#394 で 2 つ目を一般化した）──────
#
#   1. `queued` … 通報が閾値に達した作品（8.4 / #40）
#   2. **`cleared` かつ、最後に `cleared` にした時刻以降に通報が付いた作品**（#366 / #394）
#
# **2 が要るのは、`cleared` が終端だからである。** `cleared` の作品に付いた通報は
# `queued` へ上がらず、状態を見る一覧のどこにも出ない。#366 は基準を「最後の改名」に
# していたが、**それでは運営が見終えた作品が次の改名まで出続けた**ので、#394 で
# 「最後に `cleared` にした時刻」（`admin_actions`）へ変えた。**履歴の無い `cleared`
# （#361 より前に端末で `cleared` にした作品）は、通報があれば出す**（黙って落とさない）。
# 理由の全文は `src/reports.ts`。
#
# **条件は書き写さない。** 正本は `src/reports.ts` の `REVIEW_REPORTED_AFTER_CLEAR_SQL` で、この
...
# **問題なしのあとに通報が付いた作品の条件も、同じ正本から取り出す**（冒頭の 2）。
#
# 正本は 1 行の二重引用符つき文字列リテラルである（`src/reports.ts` の
# `REVIEW_REPORTED_AFTER_CLEAR_SQL`。**シェルから取り出せる形にするために**テンプレートリテラルに
...
if [[ -z "$AFTER_CLEAR" ]]; then
  echo "[queue] ${REPORTS_TS} から REVIEW_REPORTED_AFTER_CLEAR_SQL を取り出せません。" >&2
...
ATTENTION="(g.review_state = '${QUEUED}' or ${AFTER_CLEAR})"
...
# **どちらの理由で出ているか**（審査待ちか、問題なしのあとに通報が付いたか）が分からないと、
# 運営が最初にすることが変わる。
#
...
if [[ "$FORMAT" == "json" ]]; then
  ...
  echo "[queue] 問題なしのあとに通報が付いた作品も出します（src/reports.ts の REVIEW_REPORTED_AFTER_CLEAR_SQL）"
  if [[ "$COUNT" -gt 0 ]]; then
