...
| ~~**`CAPTURE_TIMEOUT_MS` 20,000 ms**~~ | **30,000 ms へ改めた**（2026-09-01。#219。根拠は 9 章）。公開時の実測は 7.9〜16.9 秒で、**最悪 16,907 ms に対する 20,000 ms の余裕は 3,093 ms（15%）しか無かった。** 同じ作品が 7.1〜16.9 秒でぶれるので、**ぶれ 1 回分に足りない。** Lambda の 60 秒より内側なのは変わらない |
...
**20,000 ms は最悪実測 16,907 ms に対して 3,093 ms（15%）しか無く、ぶれ 1 回分に足りない。**
30,000 ms は **78% の余裕**で、`ogp_function_timeout_seconds`（60 秒）より内側なので、
諦めたあとに失敗のコールバックを送る余地も残る。
