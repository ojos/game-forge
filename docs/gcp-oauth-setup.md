...
#### なぜ `game-forge.ojos.jp` ではないのか（#610。2026-09-17 に実測）

**承認済みドメインをサブドメインにはできない。** 上の 2 点（terraform の外にある / 消すと外れる）は
**このプロジェクトの怠慢ではなく、Google の制約の帰結である。**

**`game-forge.ojos.jp` にできるなら、そのほうが良い。** あちらのゾーンは Route53 にあり
（`terraform/dns.tf`）、TXT を宣言として置ける——**規範 4（外部状態は宣言的に管理する）を満たし、
「消さないこと」という人間の注意力に頼る注意書きも要らなくなる。** 許可の範囲も狭くなる。

**しかし Google が受け付けない。** 2026-09-17 に Console の「ブランディング」→「承認済みドメイン」で
`game-forge.ojos.jp` を入力したところ、次のエラーで拒否された。

```text
無効なドメイン: 最上位のプライベート ドメインを指定する必要があります
```

**承認済みドメインは eTLD+1（登録可能ドメイン）でなければならない。** Public Suffix List の判定で
`jp` が公開接尾辞、その直下の `ojos.jp` が「最上位のプライベートドメイン」にあたる。
`game-forge.ojos.jp` はそのサブドメインなので受け付けられない。
...
> **`ojos.jp` を消した状態のまま保存しないこと**——ホームページ・プライバシーポリシー・利用規約の
> URL（すべて `app.game-forge.ojos.jp`）が承認済みドメインでカバーされなくなる。試したら
> 「変更を破棄」で戻す。
