...
- 終わったときの画面には、勝ったのか負けたのかが分かる言葉と、やり直しの押し方を出します。
...
- img.Set は 1 ドットずつ置くもので、絵を作るときだけ使います。Draw の中では呼びません。
...
- 遊びが終わる条件があり、終わったときに結果とやり直しが画面へ出る（利用者の指示に終わり方が書いてあるなら、そちらに従っている）
- 画面へ出した選べるもの・押せるもの・拾えるものが、すべて遊びの何かを実際に変えている
- 初期化するだけで一度も読まない変数が残っていない
- ebiten.NewImage と img.Set を Draw の中で呼んでいない（絵は初期化時に 1 度だけ作る）
