 ...
 */
const SUCCEEDED: readonly AdminOutcome[] = ['applied', 'unchanged', 'recorded-not-queued'];
...
const OUTCOME_MESSAGES: Readonly<Record<AdminOutcome, string>> = {
  ...
  // **削除依頼の措置は上書きしない**（0018 / #406）。押し直しても変わらないことを言う。
  'already-handled':
    'この削除依頼には既に措置が記録されています。記録は上書きしません（判断を変えるときは、変えた事実ごと分かるように扱ってください。docs/takedown.md）。',
  // **記録は済んだが、止める作品が無かった**ことを言う。黙って「操作しました」と出すと、
  // 運営は新規露出が止まったと読む。
  'recorded-not-queued':
    '措置を記録しました。ただし対象の作品が見つからないか公開中ではないため、審査キューへは入れていません。',
  // **必須であることと、なぜ必須かを言う**（2.4.4。削除依頼への回答に使う）。
