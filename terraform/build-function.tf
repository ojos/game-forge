...
locals {
   ...
   *
   * 上限は**オーケストレータ（terraform/orchestrator.tf）の 600 秒**が与える。あちらは
   * 最悪ケースを「3 試行 ×（生成 91 秒 ＋ ビルド最大 T の 2 回）」と見積もっており、
   * 600 秒へ収めるには **T ≤ 54 秒**が要る（`3 × (91 + 2T) ≤ 600`）。
   * **60 秒だと 633 秒で溢れる。**
   *
   * **待つのは利用者ではなくオーケストレータである**（#160 で生成が非同期になり、
   * 利用者は作品ページで待つ）。だから伸ばせるが、**無制限ではない。**
   *
   * **45 秒は、下限 44.9 秒と上限 54 秒のあいだにある最小の 5 の倍数**である。
   *
   ...
   */
  build_function_timeout_seconds = 45
