...
**この土台の時点では、利用者が押せる退会の口は無い。** 口・画面・ログインの停止・法務文書は
#518 が足す。ここに書いた経路は、**口が来るまで何もせずに 5 分ごとに起きて終わる。**
...
**アカウントの cron は 5 本まで**（Workers Free）。2026-09-16 の時点で使っているのは 0 本で
（アカウントの Worker スクリプトは `game-forge-likes` の 1 本だけ、その `schedules` は空。
Pages プロジェクトは cron を持てない）、これが 1 本目である。残りを数えるには、上の URL の
`workers/scripts` までを叩いてスクリプトを列挙し、1 本ずつ `schedules` を見る。
