 ...
 *
 * ## 逐次でも 1 回だけはみ出す。並行ではみ出さない（#455）
 *
 * **判定は要求の手前で 1 回だけ行い、2 回目の試行の前には見直さない**（3.3-2 の 1 か所。
 * オーケストレータは枠を持たない。`src/orchestrator/pipeline.ts` 冒頭の表）。台帳の行は
 * LLM の応答が返ってから入るので、**9 回目で始めた要求がコンパイルに落ちて 2 回目の
 * 試行へ進むと、その日の呼び出しは 11 回になる。** これは仕様として受け入れる
 * （2026-09-13 の #43 で本番で 11 回を観測した。`docs/cold-start-samples.md` 6 章）。
 *
 * **1 人 1 日の上振れの上限は「{@link DAILY_QUOTA_PER_USER} ＋
 * （`MAX_GENERATION_ATTEMPTS` − 1）」＝ 10 ＋ 1 である**（`src/build-retry.ts`。
 * 整理パスは `TIDY_ATTEMPTS` ＝ 1 試行なのでこれを超えない）。成り立つ理由は、
 * **1 人が同時に走らせられる要求を 1 本に絞っている**からである。
 *
 * - 生成・フォーク・推敲の 3 経路は、その利用者に**進行中の要求**（自分の `games` の
 *   `pending` / `running`、または自分の作品の推敲ジョブの `pending` / `running`）が
 *   あれば、行を作らずに断る（{@link IN_FLIGHT_REASON}）
 * - 判定は行を作る文そのものの条件にある（`src/games.ts` の `inFlightGuardSql`）ので、
 *   **同時に届いた 2 本のうち通るのは 1 本だけ**である
 * - 進行中と見なすのは開始から `STALE_AFTER_SECONDS`（`src/games.ts`）秒以内の行だけで、
 *   **止まったまま残った行で利用者を締め出さない**
 *
 * **#455 より前は、並行に出すと「要求の本数 × 最大 2 回」まで積み上がりえた**
 * （判定から台帳への記録までを、予約も排他もしていなかった）。
 *
 * **予約による厳密化（10 回を 1 回も超えない形）は採っていない**（#455 の scope.out）。
 * 月次上限（サービス全体）は利用者をまたぐ並行を塞いでいない——こちらも scope.out。
 */
 ...
 *
 * **そこで、逃がす対象を文面ではなく「範囲」で表す。** 除くのは次の 3 つだけで、
 * どれも**この仕様書が既に使っている表記上の約束**である。
 *
 ...
 * 3. **「版の履歴」の節（末尾）。** 1 と**同じ性質のもので、置き場所だけが違う**
 *    ——#621 で版ごとの要旨を 3 行目から移した先であり、**版ごとに 1 行の「そのとき
 *    どうしたか」しか入らない。** 1 章だけを除いていたのは、**移す前の構成のまま
 *    だった**ためである（#718 で、枠の値を動かしたときに v1.113 / v1.114 の行が
 *    引っかかって分かった）。**取りこぼしは起きない**——この節は仕様書の最後で、
 *    確定事項一覧（付録）より後ろにあり、**現行値の宣言はどれもこれより前にある。**
 *
 * **いずれも「書き換えれば通る」ではなく「印を付ければ外れる」形である。** 次に
...
export function currentDeclarationsIn(spec: string): string {
  ...
  const withoutHistory =
    chapterOne >= 0 && chapterTwo > chapterOne
      ...
      : spec;
  // 「版の履歴」の節（末尾）を落とす。**見つからなければ何も落とさない**（1 章と同じ
  // 扱いで、構成が変わったことに気づけるよう黙って全部を捨てない）。
  const versionHistory = withoutHistory.indexOf('\n## 版の履歴');
  const withoutVersions =
    versionHistory >= 0 ? withoutHistory.slice(0, versionHistory) : withoutHistory;
  // 取り消し線で囲まれた部分を落とす（複数行にまたがる span も対象にする）。
  return withoutVersions.replace(/~~[\s\S]*?~~/gu, '');
}
...
/**
 * 進行中の要求があるために断ったことを表す分類名（#455）。**利用者への文言ではない。**
 *
 * **{@link QuotaRejectionReason} に入れない。** 日次・月次は「枠が尽きた」で、復帰の条件は
 * 時刻（翌日 0 時・翌月）である。こちらは**枠は残っていて、いま走っている 1 本が終われば
 * すぐ通る。** 429 の分類へ混ぜると、画面が「枠を使い切りました」と誤って言い、利用者は
 * 翌日まで待つ。**ステータスも分ける**（{@link IN_FLIGHT_STATUS}）。
 */
export const IN_FLIGHT_REASON = 'generation-in-flight' as const;

/**
 * 進行中の要求があるために断るときのステータス（#455）。
 *
 * **409 Conflict である。** 429 は日次・月次の枠切れが使っており（{@link QUOTA_EXCEEDED_STATUS}）、
 * 同じ値にすると応答を受けた側が分類名を読まない限り区別できない。こちらは「利用者の
 * いまの状態（走っている要求）と衝突している」で、推敲の「前の手直しが終わるまで
 * お待ちください」（`src/revise.ts` の `not-revisable`）と同じ性質の断りである。
 */
export const IN_FLIGHT_STATUS = 409;

/**
 * 進行中の要求があるために断ったときの見出し（#455。生成・フォーク・推敲で共有する）。
 *
 * **3 経路で同じ文言を使う。** 断る理由は「どの経路から来たか」ではなく「いま何かが
 * 走っている」ことで、走っているのが別の経路の要求（生成中に推敲を押した、など）でも
 * 利用者にできることは同じ——終わるのを待つ——だからである。
 */
export const IN_FLIGHT_HEADING = '進行中の生成があります';

/**
 * 進行中の要求があるために断ったときの本文（#455）。
 *
 * **「枠」を減らしていないことを言う。** 断ったのは LLM を呼ぶ前で、台帳に行は増えて
 * いない。言わないと、利用者は日次枠切れ（{@link DAILY_QUOTA_REASON}）と同じ意味に
 * 読み、翌日まで待つ。**「本日の枠は終了しました」「今月の生成は終了しました」を
 * 含めない**（4.4 の 2 つの停止の言い回しと混ぜない）。
 *
 * **呼び名は #513 の「フォーク」「リフォージ」である。** このモジュールはオーケストレータの束に入るが、
 * この定数は束の出力に入っていない（tree-shaking で落ちる）。書き換えの前後で `CodeSha256` が変わらないことを
 * 実測してから直した（PR #526）。
 */
export const IN_FLIGHT_BODY =
  '生成・フォーク・リフォージは 1 つずつ行えます。いま進んでいるものが終わってから、もう一度お試しください。' +
  'この操作では生成枠を消費していません。';
 ...
 * チャット（5.16）も同じ表に 1 行を積むが、**確定25 の日次 10 回は 1 回も減らさない**と決めた
 * ——チャット 1 往復は **実測 0.64 円**で、生成 1 回（実測 ¥22.41）と同じ 1 回として数えると
 * **チャットするほど作れなくなり**、「チャットしてから生成する」という #695 の目的に逆行する。
 * **チャットには別の蓋がある**（1 人 1 日 ¥20。#751 まではトークン数で数えていた。`src/chat-quota.ts`）。
 *
