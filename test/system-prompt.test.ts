...
describe('本文の構成（6.1）', () => {
  ...
  it('元のソースを渡されたときも上限が変わらないと書いている（5.3 / M5-2 / #33）', () => {
    // 5.3 のソース上限（#284 以降 64KB）は**フォーク連鎖による肥大化を止める**ために置かれている。
    // 上限を「出力の大きさ」としてだけ書くと、元ソースを渡された生成は「足すだけ」を
    // 続ける（4.2 の実測でも、出力は親の大きさに引きずられて 10,100 前後まで伸びた）。
    // **収まらないときに何を諦めるかまで書く**——書かないと、モデルは指示された機能を
    // 優先して上限を割る。
    const text = renderSystemPromptText();
    expect(text).toContain('元のソースを渡されたときも、この上限は変わりません');
    expect(text).toContain('足す機能のほうを減らします');
    // **上限の話は 1 節目にまとめて置く。** 別の節へ散ると、片方だけを直した日に
    // 食い違ったまま両方が本文に残る。
    expect(SYSTEM_PROMPT_SECTIONS[0]).toContain('元のソースを渡されたときも');
  });

  it('ソースの上限を書き写さず定数から埋める（確定18 / 5.3）', () => {
    // 数字を手書きすると、5.3 の上限を実際に検査する段を作った日にずれる。
    expect(MAX_SOURCE_BYTES).toBe(64 * 1024);
...
describe('ゲームとして成り立たせる（#597）', () => {
  ...
  it('負けの条件は既定の終わる条件に数えないと書いている（#668）', () => {
    // **版 3 までは例示に「残りが 0 になる」があり、残機や体力が 0（＝負け）とも読めた。**
    // #617 の `えらべるサカナ` は、体力 0 のゲームオーバーだけで既定を満たしたと読み、
    // クリアの無いまま延々と続いた。例示が同じ段落の「延々と続くだけにはしません」と
    // 矛盾していたので、**例示から消えていることと、負けを数えない文があることの両方**を見る。
    const text = renderSystemPromptText();
    expect(text).not.toContain('残りが 0 になる');
    expect(text).toContain('負けの条件（残機や体力が 0 になる）は、ここでいう終わる条件になりません');
    // **負けを置くこと自体は禁じない**——禁じると、負けのある遊びまで書けなくなる。
    expect(text).toContain('負けを置くのはかまいませんが');
    // 最後に読む点検表でも同じ意味にそろえる（モデルは点検表を強く守る）。
    const check = SYSTEM_PROMPT_SECTIONS.at(-1)!;
    expect(check).toContain('遊びが終わる条件があり（負けの条件だけではない）');
  });
  ...
  it('終わる条件が実際に満たせることを確かめさせる（#644。到達できない作品が出た）', () => {
    // **本番の `ネズミのカギ`（`cb7749dc`）は、扉を開けたあと次の場面へ進む条件を
    // `px > 316`（画面幅 320 / 当たり判定の半径 6）と書いており、到達できなかった。**
    // **`has_win_text` も `state_count` も「良い」と言っていた**——代理指標の限界である。
    const text = renderSystemPromptText();
    // **位置で判定するときが、いちばん外しやすい。**
    expect(text).toContain('当たり判定は自機の中心ではなく端で止まる');
    // **「通り過ぎたか」は、通り過ぎられない場所では永久に成り立たない。**
    expect(text).toContain('そこを通り過ぎたか');
    // **勧めている側も照合する**（PR #645 の Copilot の指摘）。反例だけを見ていると、
    // **肯定側の 1 行が消えてもテストが通る。**
    expect(text).toContain('その場所へ触れたか');
    expect(text).toContain('扉のマスに重なった時点で進めます');
    // **「終わる条件を置くなら」に限定されている**（同）。無条件に求めると、
    // 直前の「利用者の指示が優先」と衝突し、終わりのない遊びを指示されても終端を足す。
    expect(text).toContain('終わる条件を置くなら');
  });
  ...
  it('自己点検が、終わり方・口の結線・スプライトの 3 点を確かめさせる', () => {
    ...
    // #644: 到達できない終わり方を、最後に読む点検表でも捕まえる。
    expect(check).toContain('終わる条件を置いたなら、それが実際に満たせる');
    // **壁のマスに重なる判定まで禁じない**（PR #645 の Copilot の指摘）。扉は壁のマスに
    // 置かれるので、「壁のマスを条件にしない」と書くと**勧めている形を自分で塞ぐ。**
    expect(check).toContain('壁のマスに重なったら成り立つ形は');
  });
});
