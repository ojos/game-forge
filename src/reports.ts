 ...
 *
 * 戻る前に付いていた通報は `cleared` のまま残るので、それは
 * {@link REVIEW_RENAMED_SQL} が拾う。
 */
 ...
 * {@link REVIEW_RENAMED_SQL} が引くためで、**`src/games.ts` から import しない**
 * ——あちらが既にこのモジュールから `reviewVisibleSql` を取っており、逆向きの import は
 * 循環参照になる（`src/paths.ts` が値だけの葉に逃がしているのと同じ問題）。
 *
...
/**
 * 「`cleared` にしたあと改名され、その改名より後に通報が付いた」作品の条件（8.4 / #366）。
 *
 * ══════════════════════════════════════════════════════════════════════════════
 * なぜこの条件が要るのか
 * ══════════════════════════════════════════════════════════════════════════════
 *
 * **改名は「穏当な題名で公開 → 通報 → 審査で `cleared` → 改名」という経路を開ける。**
 * {@link REVIEW_CLEARED} は終端であり（上記「再び閾値に達しても戻さない」）、
 * {@link recordReport} は `review_state !== null` の作品で早期に return する。
 * **改名そのものは `cleared` を `NULL` へ戻す**ので（`src/games.ts` の `renameGame`）
 * 以後の通報はまたキューへ入るが、**戻す前に付いていた通報**と、**閾値に届かない
 * 通報**は `cleared` のまま残る。この条件はそれを拾う。
 *
 * ══════════════════════════════════════════════════════════════════════════════
 * 綴りを 2 か所に置かない
 * ══════════════════════════════════════════════════════════════════════════════
 ...
 *   - admin の審査キュー画面（#367） … {@link reviewAttentionSql} か、この定数を直接
 *
 * **1 行の文字列リテラルで書く。** 他の定数を差し込むテンプレートリテラルにすると、
 * シェル側が取り出せない（あちらに TypeScript の評価器は無い）。**そのぶん、綴りが
 * {@link REVIEW_CLEARED} / {@link TITLE_CHANGES_TABLE} / {@link REVIEW_STATE_COLUMN} と
 * 一致していることは `test/title-rename.test.ts` が機械照合する。**
 *
 * ══════════════════════════════════════════════════════════════════════════════
 ...
 *
 * **`reports` と `title_changes` の別名（`r` / `c`）は副問い合わせの中で閉じている**
 * ので、外側の別名と衝突しない（外側が `r` を使っていても、内側の `r` が優先される
 * ——ただし相関の対象が変わるので、**借りる側は外側で `r` を別の意味に使わないこと**）。
 *
 * ══════════════════════════════════════════════════════════════════════════════
 * 改名が 1 度も無ければ当たらない
 * ══════════════════════════════════════════════════════════════════════════════
 ...
 *
 * **{@link REVIEW_QUEUED} と {@link REVIEW_RENAMED_SQL} の和である。** 前者は
 * 「通報が閾値に達した」作品、後者は「審査を通ったあとに改名され、そのあと通報が
 * 付いた」作品で、**運営がすることは同じ**（作品ページを見て判断する）。
 *
...
export function reviewAttentionSql(): string {
  return `(g.${REVIEW_STATE_COLUMN} = '${REVIEW_QUEUED}' or ${REVIEW_RENAMED_SQL})`;
}
