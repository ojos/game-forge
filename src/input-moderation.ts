 ...
 *
 * **親ソースには当てない。** フォークのペイロードには親の `source.go` が
 * `MAX_SOURCE_BYTES` まで、**整理パス（ペイロードの版 3）ではその 2 倍まで**載る
 * （5.3 / `src/orchestrator/payload.ts`）。**ゲームのソースには `enemy` / `kill` / `shoot` / `damage` /
