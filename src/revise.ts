 ...
 * （仕様 5.7 / 確定31）。月の費用は 4.3 のサービス全体の上限が守る。
 *
 ...
 * | 失敗 | 枠 | 理由 |
 * |---|---|---|
 * | 元のソースが読めない / 上限超（確定18） | **返す** | **LLM を 1 度も呼んでいない。** 確定28 が失敗を数える根拠（費用は出ている）が当てはまらない |
 ...
 *
 * 読み取りと上限の判定は `src/source-store.ts` にある（#217）。**枠を返すのはこちらの
