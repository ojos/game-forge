...
**ヘッダとフッタ（利用者向けの全画面と管理画面）に、横組みを入れました（#440）。**
ファビコン・ホーム画面のアイコン・OGP の既定画像には、まだ入れていません（#440 の scope.out）。

### 7.1 アンバーは、ロゴの中だけの例外にした

サイトの見た目は「色を持つのは作品だけ（例外はエラーの赤）」です（`public/assets/app.css` の冒頭）。
**#440 の intake（2026-09-13）で、ロゴのアンバーを「ロゴに限った例外」にしました。** ロゴは装飾では
なくサービスの印で、火花は小さく作品の色と主役を争いません。1 色版は書き出していません。

**例外は画像の中に閉じます。** アンバーを CSS のトークンにしません——トークンにすると、ボタンや
リンクへ広がる入口ができるためです。
...
| 項目 | 決めたこと | 理由 |
|---|---|---|
| 使う画像 | `lockup-horizontal/` の x1・x2・x4 の明暗、計 6 枚 | 3 倍の書き出しは無いので、3 倍の端末は 4 倍の画像を縮小して描く |
| 表示の大きさ | **1 倍（122×31px）** | 規約のヘッダの高さに収まり、390px の画面でもナビと並べられる |
| 高密度の画面 | `srcset` で 2 倍・4 倍を選ばせる | 整数倍の画像をブラウザに選ばせる。**画素密度が整数でない端末の見え方は保証しない** |
| 明暗 | `<picture>` の `<source media="(prefers-color-scheme: dark)">` | サイトの明暗が同じ条件だけで切り替わるので、地と画像が必ず揃う（3 章の「暗い地に `for-light-bg` を置くと消える」を防ぐ） |
| 読み上げ | `alt="Game Forge"` | 文字のロゴだった頃と同じリンクの名前にする |
| 置く場所 | サイトの配信物は `public/assets/logo/` の**写し** | Pages が配るのは `public/` の下だけ。**写しが正本と一致することは `scripts/check-logo-copies.sh` が見る**（`scripts/acceptance.sh` から回る） |

**4 章で書き出し直したら、`public/assets/logo/` へも写します。** 写し忘れると `scripts/check-logo-copies.sh`
が落ちます。
