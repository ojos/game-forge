 ...
 * 5 … #730 で、**1 フレームで必ず制御を返すこと**を 2 節へ、**終わり方を持たない繰り返しを
 *     書かないこと**を 2 節と 8 節の自己点検へ足した本文。版 4 までのどの節も、**1 フレームの
 *     中で何が起きるかに触れていなかった**——`for {}` を書いた作品は、遊ぶ人のタブごと
 *     固める（仕様 7.2 の実測。ページの中に回復手段が無い）。**同じ形は生成後の検査
 *     （`src/go-loops.ts`）が拒否する**ので、この版から先は**プロンプトと検査の両方**が
 *     見ている。
 */
