 ...
 *
 * > **#681 注記（2026-09-18）。上の理由 1 の「定期実行の置き場所は、まだ無い」は、もう事実ではない。** 置き場所は
 * > `game-forge-cleanup`（cron 5 分ごと。#586 / #518 が退会の後続の処理のために置いた）で、#681 はそこに
 * > **止まったまま残った生成・推敲の行を `failed` に畳む処理**を足した（`src/stale-generation-sweep.ts`。区切りは
 * > 1 時間）。畳まれた作品はこの一覧の「失敗」に入り、作者は既存の削除で消せる。**#242 が「止まった行は 3.7 の
 * > 掃除に任せる」と決めた前提は、この畳みで置き換わった。** 一方、**14 日の自動削除は今も動いていない**ので、
 * > 残り日数を出さない理由 1 そのものは変わらない。
 *
 ...
 *
 * **正本は `src/works-paths.ts` である**（ヘッダや一括操作など、ほかの画面もこの綴りを読むため、
 * 値だけの葉に置く。`src/paths.ts` に置かない理由は、Lambda の束に入る
