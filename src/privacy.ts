...
<ul>
  ...
  <li><strong>相談のときに、いつも守ってほしいこと</strong>（登録情報の「相談」に書いた場合）: あなたが書いた文。<strong>あなただけが見られます（公開しません）。</strong>相談のたびに、いちばん最初に AI へ伝えるために保存します。<strong>相談にだけ使い、生成そのものには渡しません。</strong>登録情報の「相談」で書き換え・削除（空にする）ができます。Cloudflare のデータベース（D1）に保存します</li>
...
<ul>
  ...
  <li><strong>Amazon Web Services（AWS）</strong>: 作品の生成・ビルド・紹介用の画像の撮影と、アイコンの画像の作り直しを行う処理の実行（アイコンの画像は、作り直すあいだだけ送り、AWS 上には保存しません）。
    <ul>
      ...
      <li>生成の前の相談でも、同じ <strong>Amazon Bedrock</strong> と <strong>Amazon Bedrock Guardrails</strong> を使います。送るのは、あなたが相談で書いた文と、<strong>あなた自身の作品の題名と指示文</strong>です（あなたが求めたときだけ、あなた自身の作品のソースコードも送ります）。登録情報の「相談」に<strong>いつも守ってほしいこと</strong>を書いている場合は、その文も毎回送ります。<strong>ほかの方の作品は送りません。</strong></li>
    </ul>
  </li>
...
<ul>
  ...
  <li><strong>相談のときに、いつも守ってほしいことは、期限を定めた自動の削除を行いません</strong>（設定として置き続けるものです）。登録情報の「相談」で空にすると、その時点で削除します。退会したときも削除します（下の「退会」）。</li>
