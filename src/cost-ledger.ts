...
// **リトライの上限は借りる。** 同じ数を A/B の集計へ書き写すと、5.2-7 の上限を
 ...
 *
 * - **4.3 の枠が 150 円/ドルの上に建っている。** 日次クォータ 10 回は
 *   「¥667/日 ÷ ¥22.41」から逆算され、その ¥22.41 は 4.2 の本番実測を 150 円/ドルで
 *   換算した値である。台帳だけ別のレートにすると、**枠を決めた値と枠を判定する値が
 *   ずれ**、4.3 の「層 3 が発火したら台帳のずれ」という判定基準が成立しなくなる。
 * - **差は 3.3% で、層 3 のバッファに収まる。** 層 3 は 170 USD（130 円/ドルでも
 *   約 22,100 円）で、**層 1 より先に発火してはいけない**という向きに置かれている。
 *   2 万円は 150 円/ドルで 133.3 USD なので、実勢が 155 でも 160 でも層 1 が先に出る
 *   ——**台帳はこの定数（150）で割るので、層 1 の発火点は実勢が動いても 133.3 USD で
 *   変わらない。** 層 3 が測るのは実支出の USD で、そちらは 170 である。
 ...
 *
 * **上の 3 点に出てくる周辺の値は #284 で入れ替わっている**（日次 12 → 10 回、
 * 単価 ¥9.9 → 実測 ¥22.41、層 1 の上限 1 万 → 2 万円、層 3 の予算 85 → 170 USD）。
 * **レート 150 とこの論証の骨は変わっていない**——層 3 と層 1 の比は 1.275 倍のままで、
 * 上限と予算が同じ比で動いたことがそれを示している。**照合しているのは仕様書の
 * 「為替は N 円/ドル」だけなので、この説明文は機械では守られない**（`EXCHANGE_RATE_PATTERN`
 * が読むのは `docs/product-spec.md` であって、このコメントではない）。**値を動かした
 * ときは、ここを人が読んで直すしかない。**
 */
 ...
 *
 * **「為替（レート）は N 円/ドル」の形だけを対象にする。** 4.3 の「170 USD は
 * 130 円/ドルでも約 22,100 円」は**意図的に別の値**（層 3 が層 1 より先に発火しない
 * ことを示す円高側の確認）であり、換算レートの宣言ではない。ここまで拾うと、正しい
 * 記述で赤が出る。**#284 で層 3 が 85 → 170 USD へ動いても、この形（130 円/ドル）は
 * 変わらない**——除外しているのは値ではなく文の形である。
 *
 ...
 * これしか無い——**4.3 の月次 2 万円は全部を合算する**（総額にはチャットも効く）が、
 * **確定25 の日次 10 回は生成の行だけを数える**（チャットは日次枠を 1 回も減らさない。5.16）。
 * 別の表へ分ければ前者が崩れ、列を足さなければ後者が崩れる。
 *
 * **数える側が絞る。** この定数を読むのは、日次枠（`src/quota.ts` の `dailyCallCount`）と
 * チャットの枠（`src/chat-quota.ts`）である。**月次累計（{@link monthlyCostTotals}）は絞らない**
 * ——絞らないことが 4.3 の決定そのものなので、条件を書かないほうが意図が読める。
 */
export const GENERATION_KIND = 'generation' as const;

/** 台帳の行の種別のうち、チャット（5.16）。 */
...
export async function recordGeneration(
  ...
  entry: {
     ...
     *
     * **チャット（5.16）だけがここへ {@link CHAT_KIND} を渡す。** 既定を生成にしてあるのは、
     * **この列より前から在る呼び出し元が 1 つ残らず生成だから**である（チャットは存在しなかった）。
 ...
 *
 * **利用者で絞らない。** 4.3 の月次 2 万円は**サービス全体**の上限である。1 人あたりの
 * 蓋は日次クォータ 10 回（確定25）で、そちらは #23 が持つ。
 *
 * **判定はここでしない。** しきい値（2 万円・80% 警告）と停止は 3.3-2 の段（#23）の
 ...
 * ~~「推敲（19.5〜25.0 円）と新規生成（約 16 円）は別の値」~~と定めていた。混ぜると
 * `effort` の効果より大きな差がそこから入る、という理由である。
 *
 * **→ 数え直すと「別の値」は成り立たなくなった（#296）。** 新規生成は
 * **¥22.41**（2026-09-04 / 本番の既定群 20 件の平均。4.2 の実測注記）で、推敲の帯
 * （19.5〜25.0 円 / 平均 約 21.4 円。2026-08-30〜31 の n=4）と**重なっている。**
 *
 * **境目を残す理由のほうは強くなった。** 帯が離れていた頃は、集計に推敲が混ざれば
 * **値の大きさで気づけた**。いまはそれが利かない——**平均で 1 円しか違わないものが
 * 混ざっても、目では分からない。** そして混ぜてはいけない理由自体は変わっていない。
 * 推敲・フォークの費用は**元ソースの大きさ**に引きずられ（1.2.43。出力トークンにほぼ
 * 比例）、そこへキャッシュ書き込みの 1.25 倍が乗る。**`effort` の効果より大きな差が
 * そこから入る**のは前と同じで、**気づく手段だけが消えた。** だからこの境目は
 * 費用ではなく**トークンで**引く（下記）。
 *
 ...
 * 1 依頼は最悪でも約 14 分に収まる（`scripts/check-orchestrator-retry.sh` が計算する
 * 829 秒。生成 297 秒 × `MAX_GENERATION_ATTEMPTS` ＋ ビルド ＋ 余裕）。
 * **30 分に散らばった同一プロンプトの行は、別々の依頼である。**
 *
 * **#284 で 1 依頼の長さが 3〜4 倍になった**（生成 91 → 297 秒。試行は 3 → 2）。
 * **窓の 30 分は動かしていない**が、余裕は約 14 倍から約 2 倍へ縮んでいる。
 * **次に生成の秒数を伸ばすときは、この窓も一緒に見直すこと。**
 */
...
export interface EffortExperimentGroup {
   ...
   * **依頼の切り分けが怪しいまとまりの数。**
   *
   * 同じプロンプトの行が 5.2-7 の上限（{@link MAX_GENERATION_ATTEMPTS} 試行）を
   * 超えている、または
 ...
 *
 * **前提が崩れたら見えるようにしてある。** 同じプロンプトの行が 5.2-7 の試行上限を
 * 超える、または 30 分より広く散らばっていれば `ambiguousJobs` が立つ。
