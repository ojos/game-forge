 ...
 * 依存が生まれない。
 *
 * **このファイルはオーケストレータ Lambda の束に入る**（{@link WORK_PAGE_PREFIX} の節）。
 * 足した値が束に残ると、Lambda が読まない値でも `CodeSha256` が変わり、#241 の関門が
 * 本番配備を止める。**画面だけが読む綴りで、値が式になるもの**（別の定数を参照する
 * テンプレートリテラルやメソッド呼び出し。束が落とせずに残る）は `src/works-paths.ts`
 * のように別の葉へ置く（#328 で 1 度止めた）。
 */
 ...
 * 本文の推測で分けない**（前者は 1 回 ¥22.41、後者は 0 円である。費用は 2026-09-04 /
 * 本番の既定群 20 件の平均。4.2 の実測注記）。
 */
