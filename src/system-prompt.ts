...
- 終わる条件が、実際に満たせることを確かめます。条件に使う数や位置が、遊びの中で届く範囲にあるかを見ます。届かない条件を書くと、遊ぶ人はそこまで行ったのに何も起きない、という状態になります。
...
- 終わる条件が実際に満たせる。位置で判定しているなら、その線が行ける範囲の内側にある（画面の端や壁のマスを条件にしていない）
