 ...
 * | 作品ページを開いたら回収する | **GET が状態を書き換える形にしない**（`src/games.ts` の `STALE_AFTER_SECONDS`。作品ページ `src/work-page.ts` はそれを再 export して読む）。ページを開いた人が行を壊せることになる |
 ...
 * （`generations`）に行は増えず、**確定25 の日次 10 回にも当たらない。** 公開そのものが
 * 枠を使わないのと同じ扱いである。
 *
 * # CSRF について
 *
