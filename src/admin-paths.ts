 ...
 * フォームの項目名（**次の状態**）。
 *
 * 審査は `queued` / `cleared`、BAN は `banned` / `cleared`……とはせず、**それぞれの口が
 * 自分の語彙で受ける**（`src/admin/review.ts` / `src/admin/users.ts`）。**「いまどちらか」
 * ではなく「どちらにしたいか」を送る**——画面を開いたまま別の管理者が動かした場合に、
 * 「切り替える」だけの要求だと**送った側が意図しない向きへ倒れる。**
 */
