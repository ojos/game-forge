 ...
 * 画面の側が付ける。見た目（画面の器と同じ幅で左端から組む。#761）は app.css の `@section legal` が持つ。
 *
 ...
 * cookie が無いので 1 行も読まない。**D1 が落ちても投げない**——読めなければ
 * **「ログイン済みのまま」へ倒し**、本文は返す（下の「投げない」と同じ理由。`/terms` と
 * `/takedown` はいちばん落としてはいけない）。**未ログインへ倒さない**のは、D1 の一時的な
 * 不調で全画面のヘッダからログアウトが消えるほうが害が大きいからで、漏れないのは上のとおり
 * （リンクの先は `resolveSessionUser` が改めて見る）。倒す向きの根拠は
 * {@link viewerStillSignedIn} に書いてある。
 ...
 *   親へのリンクの見せ方ではなく、パンくずを本文と同じ読み物の器に載せるための印である
 *
