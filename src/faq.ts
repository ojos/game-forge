...
<p>アカウントごと消したいときは<a href="#withdraw">退会できますか？</a>をご覧ください（退会すると、指示文も削除します）。</p>`,
  },
  {
    id: 'withdraw',
    question: '退会できますか？',
    answer: `<p><strong>できます。</strong>ログインしたうえで、設定の「アカウント」から「退会について確かめる」を開いてください。
...
<p>作品を遊ぶには、JavaScript と WebAssembly が有効になっている必要があります。</p>`,
  },
  {
   ...
   あなたの作品の一覧・状況・ソースと残りの<a href="#quota">生成枠</a>を読んだり、作品名・説明・タグを書き換えたり、新しい作品の生成とリフォージを始めたりできます。
   <strong>公開・削除・退会はできません</strong>（作品ページと設定から行ってください）。</p>
...
<p><strong>接続をやめるときや、心当たりのない接続を見つけたときは</strong>、設定の「<a href="${ACCOUNT_APPS_PATH}">接続中のアプリ</a>」で「接続を解除」を押してください。
 ...
 *   下線を常に出す（2.5.5「文章の中」）
 * - **質問と質問の間の罫線は器の端まで**（`.gf-faq-item` の上の線）。**答えの本文も読み物の器いっぱいに組む**
 *   （#761。2.5.3）
 * - **器は読み物の器**（`READING_CLASS`。画面の器と同じ幅で左端から組み、一覧のブロックと罫線もその端まで。仕様 2.5.3 / #564 / #761）
 *
 * @param entries 質問の一覧
