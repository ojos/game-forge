...
## #406（削除依頼の一覧と措置の記録）を出すときに増える手順

**画面が 1 枚増える**（`/takedowns`。ヘッダのナビに「削除依頼」）。**口も 1 本増える**
...
- ヘッダのナビに「削除依頼」があり、`/takedowns` が 200 で開くこと
- 操作の履歴（`/actions`）に、適用前の行がそのまま並んでいること（同じ秒の並びも変わらない）
- **確認のためだけに削除依頼や措置を作らない**（Ⓒ〜Ⓓ と同じ理由。記録は依頼への回答に使う正本である）。
