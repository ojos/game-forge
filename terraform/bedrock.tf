 ...
 * 中へ移すことで外れる。鍵（`BEDROCK_AWS_*`）は削除する対象であって、ローテーションする
 * 対象ではなかった（docs/orchestrator.md）。**削除は済んでいる**——本番の Pages の
 * シークレットに残るのは `BUILD_AWS_*` の 3 つだけである（2026-09-15 に読み取りで確認）。
 *
 ...
 *
 * ## ユーザーを消すときにやったこと（#160 の移行で実施済み）
 *
 * **この節は履歴である。** `game-forge-bedrock-invoker` は宣言からも本番の IAM からも
 * 消えている（2026-09-15 に `NoSuchEntity` を確認。読み取りのみ）。同じ形のユーザーを
 * 消す場面が再び来たときのために手順だけ残す。
 *
 * **アクセスキーはこの宣言が持っていなかった**（理由は terraform/build-invoker.tf の
 * 「アクセスキーを宣言しない理由」。同じ判断が `BUILD_AWS_*` にもいまだ当てはまる）。
 ...
 * （docs/local-dev.md 2 章）。
 *
 * **#160 より前はここに「長命キーが要るのは Workers が動く本番だけである」と書いていた。**
 * 今は Bedrock 用の長命キーがどこにも要らない（prod も実行ロールで呼ぶ）。本番のエッジに
 * 残る長命キーは `BUILD_AWS_*` の 1 組だけで、それは Bedrock ではなく Lambda の呼び出し用
 * である（terraform/build-invoker.tf）。
 *
