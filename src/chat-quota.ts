 ...
 * ソースを渡さない 1 往復は**実測 1,710 トークン**だが、64 KiB のソースを 1 回渡すと約 19,200 トークンに
 * なる（**1 日の蓋を単独で超える**）。
 ...
 *    **1 日の蓋（15,000）を 2 倍以上超える。** 見積もりは高い側へ倒す（4.3 の「迷う側は高い側へ倒す」）。
 *    **#718 で蓋を半分にした結果、64 KiB のソースを渡す相談は必ず断られる**（短い発話でも見積もりは
 *    約 24,600 トークン）。**渡せるソースは約 35 KiB まで**である（`CHAT_SOURCE_BYTES_PER_TOKEN`）。
 ...
 *    最大の往復 2 本ぶんで **約 38 円**、相談の当月の取り分（1,000 円）の **3.8%** である。
 ...
 *
 * **4.3 と同じ形で、本番の実測（0.371 円/1,000 トークン。仕様 5.16 の「実測」）から引いた値である。**
 *
 *   15,000 × 0.371 ÷ 1,000 ＝ 約 ¥5.6/人・日 × **3 人**（4.3 と同じ想定）× 30 日 ＝ 約 ¥501
 *
 * となり、{@link CHAT_MONTHLY_COST_LIMIT_JPY} の半分に収まる。**2 つの値は独立ではない**
 * ——片方だけを動かすと逆算が合わなくなる。
 *
 * ## 30,000 から半分にした（#718。2026-09-20 / 利用者の決定）
 *
 * **引いたときの試算が 2 倍近く高かった。** 枠はキャッシュが効かない安全側の単価
 * 0.675 円/1,000 トークン（入力 2,800・出力 400）で引いており、**仕様 5.16 は「1 日 30,000 トークンは
 * 約 9 往復」と書いていた。** ところが**実測は 1 往復 1,710 トークン**で、30,000 では**約 17.5 往復**
 * 使えていた——**引いたときの意図と、実際に効いている蓋が食い違っていた。**
 *
 * **緩めたのではなく、意図どおりの位置へ戻した**（15,000 ÷ 1,710 ＝ 約 8.8 往復）。
 *
 ...
 * ある以上、呼びかけでは守れない）。
 */
export const CHAT_DAILY_TOKEN_LIMIT = 15_000;

/**
 * 相談に割り当てた当月の取り分（円。仕様 5.16。利用者の決定）。
 *
 * **4.3 の月次上限 2 万円の 5% である。** 4.3 の判定は今までどおり台帳の全部を合算する
 * （総額には相談も効く）ので、**これはその内側にもう 1 枚だけ置く蓋**である。
 *
 * **当たっても生成は止まらない。** それが #695 の constraints（「相談の費用が生成の予算を
 * 食わないよう、別枠で止まる」）に真っ向から答える唯一の形である。
 *
 * **2,000 円から半分にした**（#718。{@link CHAT_DAILY_TOKEN_LIMIT} と同時。**片方だけ動かすと
 * 逆算が合わなくなる**ので、2 つはいつも一緒に動く）。
 */
export const CHAT_MONTHLY_COST_LIMIT_JPY = 1_000;
