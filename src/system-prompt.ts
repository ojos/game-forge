 ...
 * 3. **ゲームとして成り立たせる** — 終わり方と、画面へ出した口の結線（#597。下記）。
 * 4. **禁止事項** — 外部ファイルロード / `fmt` / `reflect` / cgo / `//go:` 指示（6.1）。
 * 5. **許可パッケージ** — `renderAllowlistSection()` の出力をそのまま埋める。
 * 6. **許可 API の使い方** — #7 の実測で必須と分かった部分（下記）。
 * 7. **著作権と命名** — 6.2 の命名規制。
 * 8. **出力前の自己点検** — 上の各節を短い確認項目へ畳んだもの。
 *
 ...
 *
 * 6 節（許可 API の使い方）は装飾ではなく**必須**である。#7 の実測では、6.1 の制約と
 ...
 * 無い。**
 *
 * 6 節に書く API の形は、可能な範囲で **`docker/isolated-build/sample/ebitengine.go`
 ...
 *
 * ## 3 節を新設した理由（#597 / 2026-09-17）
 *
 * **同じことが「ゲームとしての成立」でも起きていた。** 3 節を作るまで、遊びの中身に
 * 触れていたのは 2 節の最後の 1 行（「遊べる状態にします。操作でき、得点か勝敗があり、
 * 初期化だけで終わらないこと」）だけで、**自己点検の 6 項目はすべて整形・import・
 * 漢字・バイト数だった**（遊びの中身は 0 項目）。モデルは最後に読んだ点検表を守るので、
 * 「コンパイルが通ること」だけを念入りに守る形になっていた。
 *
 * **基準線（公式サンプル 10 本の静的解析。2026-09-16。`docs/cold-start-samples.md` 7 章）**:
 *
 *   - **勝ちの表示があるのは 5/10。** 残りは負けるまで続くだけである。とくに「泡で
 *     閉じ込めて割ると倒せる」単画面アクション（8）は、状態が「遊んでいる」「ゲーム
 *     オーバー」の 2 つしかなく、ステージもクリアも存在しなかった
 *   - **書かれるだけで読まれないフィールドを持つ作品が 7/10。** 「宣言したが結線しない」
 *     書き癖が、画面へ出した口が効かない形の温床になる
 *   - **`ebiten.NewImage` の利用は 0/10**（下記）
 *
 * **終わり方は「指示が優先、書いていないときだけ既定」とする**（利用者の決定。2026-09-17）。
 * どの作品にもクリアを要求すると、もぐら叩きのようにクリアが本来不自然な骨組みへ
 * 不自然なゴールが付く。**判定はモデルへ委ねる**——プロンプトは静的なので、指示に
 * 終わり方が書かれているかをこちらで判定する場所が無い。
 *
 * **負けの条件は既定の終わる条件に数えない**（#668）。版 3 までは例示に「残りが 0 になる」を
 * 挙げており、これは残機や体力が 0 になる（＝負け）とも読める。#617 で遊んだ
 * `えらべるサカナ`（`245d6cb2`）は、終わる経路が `hp <= 0` のゲームオーバー 1 つだけで、
 * **例示のとおりに書いた結果、同じ段落の「負けるまで延々と続くだけにはしません」を破っていた。**
 * 例示を勝ち・到達・時間切れに限り、負けは数えないと明記した。
 *
 * ## スプライトを教える理由（#597）
 *
 * **スプライトは前から許可の内側にあったのに、1 本も使われていなかった。**
 * `github.com/hajimehoshi/ebiten/v2` は許可済みで、そこに `NewImage` / `Set` /
 * `DrawImage` / `DrawImageOptions{GeoM, ColorScale}` がある（v2.9.9 で確認）。外部
 * ファイルを読まないので 4 節の全面禁止にも抵触しない。**それでも 10 本とも毎フレーム
 * `vector` で図形を直接描いていた。** 6 節が `vector` の 4 関数しか教えておらず、しかも
 * 「便利関数を思い付いても使いません」と閉じているため、モデルがこの道に入れない。
 *
 * **許可パッケージを増やす前に、まず教える。** 素材パッケージ（`jpfont` と同じ形で
 * 焼き込む `gameforge.local/sandbox/gfx`）を作るのは、これを教えたうえでまだ足りないと
 * 分かってからでよい。
 *
 * ## モデルごとに出し分けない（決定と理由）
 *
 ...
 *    確かめる手段が無い。捏造対策は 6 節（実在する API の明示）として**共通側**へ入れた。
 ...
 * サンプルで実際にコンパイルが通っている。**
 *
 * ## 「1 フレームで必ず制御を返す」を書く理由（#730 / 2026-09-20）
 *
 * **`Update` が戻らない作品は、遊ぶ人のタブごと固まる**（7.2 の実測）。同一サイトの
 * sandboxed iframe は OOPIF にならないため、作品は親ページと同じ主スレッドに乗り、
 * **親の「閉じる」も効かなくなる。** 外へは何も送れない（7.2 の `connect-src` は
 * その作品の `.wasm` 1 本だけ）ので、残る害がこれである。
 *
 * **ここに書くことを防御と数えない**（4 節の注記と同じ）。プロンプトは利用者に対して
 * 何も保証しない。**機械で止めるのは `src/go-loops.ts`**（`findUnboundedLoops`）で、
 * ここに書くのは**善意の生成物がうっかり踏むのを避けるため**である——検査に当たると
 * 拒否になり、**再生成には回らないので利用者の枠を 1 つ消費する**（5.2-5）。
 *
 * **検査が落とす形より広く書く。** 検査は字句から確実に読み取れる 2 つの形しか
 * 落とさないが（6.1 の表）、プロンプトの側は「回る回数が決まっている繰り返しだけ」と
 * 求める。**逆向きにはしない**——プロンプトが検査より狭いと、指示どおりに書いた
 * 生成物が拒否される。
 */
...
- 上の &Game{} には、下の 3 節で持たせる画面の状態も入れます（&Game{state: stateTitle}）。
- Update と Draw は、1 回の呼び出しで必ず戻ります。1 フレームの中で回り続けると、遊ぶ人の画面が止まり、タブを閉じる以外に戻す手段がなくなります。
- 終わり方を持たない繰り返しを書きません（for {} や for true {} は拒否されます）。繰り返しは、回る回数が決まっているものだけにします（for i := 0; i < n; i++ や、スライス・配列・マップを回す for range）。
- 何かを待つときは、その場で回して待ちません。待っていることを状態として持ち、次のフレームで続きを見ます（経過はフレーム数で数えます）。`;

/**
 * 3 節: ゲームとして成り立たせる（#597）。
 *
 * **2 節の最後にあった 1 行（「遊べる状態にします。…」）をここへ移し、書き方まで示す形へ
 * 広げた。** 制約文だけでは守られないことは #7 の実測で分かっている（モジュール冒頭）。
 *
 * **状態の形をコードで示すのは、終わりが「画面の状態」として表れるからである。**
 * 終端の状態を持たない作品は、条件へ達しても描画が変わらず、遊ぶ人からは何も起きて
 * いないように見える。基準線の 10 本中 5 本がこの形だった。
 *
 * **`reset` と `cleared` は、この節でしか出てこない名前である。** 形を示すための
 * 呼び名で、6 節の API の形とは違って**実在を機械照合しない**（API 面ではないため）。
 * ただし `const ... iota` の状態と `switch g.state` は、サンプルにも同じ形を置いて
 * コンパイルが通ることを確かめている（`docker/isolated-build/sample/ebitengine.go`）。
 *
 * ## タイトル画面を「文字だけの画面」にさせない（#597 の自己レビュー）
 *
 * **状態を分けさせると、全作品がタイトル画面から始まる。** OGP の撮影
 * （`docker/ogp-shot/index.mjs`）は**ローダーが消えた 1.5 秒後の 1 枚を撮る**ので、
 * **何もしないと見本の画像が全部タイトル画面になる。** 拡散の着地点（2.3）が文字だけの
 * 画像になると、何のゲームか分からないまま共有される。
 *
 * **そこで「遊ぶ場面を描いたうえに文字を重ねる」と書く。** 状態を分けることと、最初の
 * フレームに絵があることは両立する。3.9 の実機確認と同じで、**ここは生成物の側でしか
 * 直せない**（撮影を遅らせても、タイトルで止まっている作品は動き出さない）。
 */
const GAME_DESIGN = `ゲームとして成り立たせる:
- 遊べる状態にします。操作でき、得点か勝敗があり、初期化だけで終わらないこと。

終わり方:
- 利用者の指示に終わり方が書いてあるなら、そのとおりにします。書いていないときだけ、下の既定を入れます。
- 既定の終わり方: 遊びが終わる条件を 1 つ決め、そこへ達したら遊びを止め、結果を出し、やり直せるようにします。
- 終わる条件は、指示された遊びに合うものを選びます。敵を全部倒す・決めた面をこえる・持ち時間が尽きたら結果を出す、のどれでもかまいません。負けの条件（残機や体力が 0 になる）は、ここでいう終わる条件になりません。負けを置くのはかまいませんが、負けるまで延々と続くだけにはしません。
- 画面の状態を分けて持ちます。下は既定の終わり方を入れるときの形です。利用者が別の終わり方を指示したときは、その終わり方を表す状態を、同じように分けて持ちます。
- 状態の定数は、ほかの定数と混ぜず、それだけの const のまとまりに書きます。iota は、その const のまとまりの中で何番目の宣言かを表します。画面の大きさや音の設定を先に 3 行書いてから状態を並べると、stateTitle は 0 ではなく 3 になります。
- そのうえで、g を作るときに状態を明示的に入れます。構造体の初期値は 0 なので、入れ忘れると stateTitle が 0 でないときにどの case にも入らず、Update が毎フレーム何もしません。画面は止まったままで、キーもタップも一切効かなくなります。
	...
	case statePlaying:
		// 遊びの処理。終わる条件に達したら g.cleared を決めて g.state = stateOver にします。
	case stateOver:
		if inpututil.IsKeyJustPressed(ebiten.KeySpace) {
			g.reset()
			g.state = statePlaying
...
- 終わる条件を置くなら、それが実際に満たせることを確かめます。条件に使う数や位置が、遊びの中で届く範囲にあるかを見ます。届かない条件を書くと、遊ぶ人はそこまで行ったのに何も起きない、という状態になります。
- 位置で終わりを判定するときは、行ける範囲の内側に線を引きます。当たり判定は自機の中心ではなく端で止まるので、画面の端や壁のマスを条件にすると、中心の座標はそこまで届きません。
- 位置の判定は「その場所へ触れたか」で書きます。「そこを通り過ぎたか」で書くと、通り過ぎられない場所では永久に成り立ちません。次の場面へ進む扉なら、扉のマスに重なった時点で進めます。
- Draw も同じ 3 つで分けます。状態を分けずに 1 つの画面だけを描くと、終わっても画面が変わりません。
- 終わったときの画面には、勝ったのか負けたのかが分かる言葉と、やり直しの押し方を出します。案内した押し方 1 回で、実際に遊びが始まるようにします。「もういちど」と書いておきながら、先にタイトルへ戻って 2 回押させる形にしません。
- 最初の画面には、操作の仕方と、どうすれば終わるかを短く出します。
- 最初の画面と終わったときの画面でも、遊ぶ場面（背景・自機・敵・地形）をそのまま描いたうえに文字を重ねます。何も無い画面に文字だけを置きません。作品の見本の画像は最初のフレームから作るので、そこが文字だけだと何のゲームか分かりません。

画面へ出した口は、必ず働かせる:
- 選べるもの・押せるもの・拾えるものを画面へ描いたなら、それが遊びの何かを変えるところまで書きます。
- 変わるのは見た目だけでは足りません。動きの速さ・攻撃の形・当たり判定・進める場所・残りの数のどれかが実際に変わるようにします。
- いま選ばれているものを画面へ出し、選んだ効果が出た瞬間が遊ぶ人に見えるようにします。
- 働かせないものは描きません。使う予定で置いただけの変数や、初期化するだけで一度も読まない変数を残しません。`;
 ...
 *    使い方」であって、禁止事項の節（4 節）に置くと使い方から離れる。**
 *
 * **プロンプトは防御ではない**（4 節の注記と同じ）。「漢字を使わない」が守られなかった
...
ドット絵（スプライト。外部ファイルは読まず、絵をコードで持ちます）:
...
- 絵は文字列で持ち、初期化時に 1 度だけ ebiten.NewImage へ焼いて構造体に持たせます。毎フレーム作りません。
- img.Set は 1 ドットずつ置くもので、絵を作るときだけ使います。Update からも Draw からも呼びません。
- 文字列の 1 文字が 1 ドットです。パレットに無い文字（上の . ）は置かれず、そこは透けます。行の長さはすべて同じにします。
- GeoM は書いた順に掛かります。回すときと拡大するときは、先に GeoM.Translate で絵の中心を原点へ寄せ、Rotate と Scale をしてから、最後に置きたい場所へ Translate します。
- GeoM.Scale に負の数を渡すと反転します（GeoM.Scale(-1, 1) で左右反転）。向きの違う絵を別に作りません。
- ColorScale.ScaleWithColor で、同じ絵を色違いとして使い回せます。敵の種類ごとに絵を作り直しません。
- ドットのまま大きくなるので、8x8 くらいの小さな絵を 3〜4 倍へ拡大して使います。
- 背景・床・枠のような大きな面は、これまでどおり vector で塗ります。スプライトにしません。
...
- 鳴らし直しに player.Rewind と player.SetPosition（古い名前の player.Seek も同じものです）は使いません。音源が io.Seeker ではない player に呼ぶと、その場で panic して画面が固まります。鳴らし直すときは、上のとおり新しい player を作ります。
...
- 終わり方が、利用者の指示どおりになっている。指示に終わり方が書かれていなかったときだけ、遊びが終わる条件があり（負けの条件だけではない）、終わったときに結果とやり直しが画面へ出る
- 状態の定数が、ほかの定数と混ざらない const のまとまりにある。そのうえで g を作るときに状態を入れている（入れ忘れると、どの case にも入らず何も動かない）
- 終わる条件を置いたなら、それが実際に満たせる。位置で判定しているなら、自機の中心が行ける範囲の内側で成り立つ（壁のマスより外へ中心を進めないと成り立たない条件になっていない）。壁のマスに重なったら成り立つ形は、中心が行ける範囲で成り立つので問題ありません
- 画面へ出した選べるもの・押せるもの・拾えるものが、すべて遊びの何かを実際に変えている
- 初期化するだけで一度も読まない変数が残っていない
- ebiten.NewImage と img.Set を、絵を作るときだけ呼んでいる（Update からも Draw からも呼んでいない）
- text と vector の呼び出しが上の形と一致している
- 画面へ出す文字列に漢字が混ざっていない（フォントに漢字は入っていません）
- 繰り返しがすべて回る回数の決まったもので、終わり方を持たない繰り返し（for {} / for true {}）が 1 つも無い
