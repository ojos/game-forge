...
**本番のクライアントに置くのは、この app 用と下の admin 用の 2 本だけです**
（2026-09-14 から。ローカル用の URI は開発用のプロジェクト `ojos-game-forge-dev` の
クライアントにあります。手順書の追随は #479）。

> **注記（#478）。旧記述**——「（ローカル用の既存 URI は消さないこと）」。本番の同意画面を
> 本番環境へ切り替えてブランド確認を出すときに、所有を証明できない `localtest.me` を本番の
> 承認済みドメインから外し、ローカル用の URI を開発用のプロジェクトへ移しました。
...
**招待した相手を Console へ登録する作業はありません。** 本番の同意画面は、2026-09-14 から
外部・本番環境で運用しています（仕様書 8.1「Google OAuth を本番環境で運用する」。根拠は
利用者の確認で、同意画面の状態は API から読めません）。**招待はアプリ側の招待コードだけです。**

> **注記（#478）。2026-09-14 までの旧記述**——「**テストユーザーの登録も要ります。** 同意画面は
> Testing のまま運用するため、招待した相手のメールアドレスを Console のテストユーザーへ手登録
> しないと、Google のログイン画面にすら到達できません（仕様書 8.1「Google OAuth を Testing の
