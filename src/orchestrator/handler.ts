 ...
 * ## 基盤のリトライは 0 である
 *
 * 5.2-7 が既に最大 2 試行を持っている（#284 で 3 → 2）。掛け算にすると**最大 6 回・
 * 約 134 円・日次枠 6 個**が 1 回の送信から出る（既定の 2 回リトライ＝3 配信 × 2 試行、
 * 実測 ¥22.41）。**試行が減っても掛け算になること自体は変わらない。**`MaximumRetryAttempts=0` は
