...
`readPendingCapture` が `ogp_state='capturing'` を要求し、公開済みの行は `ready` だからである。
**照合は `BUCKET.put` より前に置かれている**ので（`src/ogp.ts`）、この場合は R2 も書かれない。
...
- **掴み直すとトークンが差し替わる。** 遅れて届いた 1 通目のコールバックは 404 で弾かれる。
  **照合を通った後だったなら R2 へは書かれるが、鍵は撮影ごとに違う**ので（#640）、行が指している
  画像には当たらず、**弾かれた側が自分の分を消す**
