...
**「問題なしとしたあと、改名されて通報が付いた作品」** を置き、行の頭に
**「改名後に通報あり」** の札と**最終改名の時刻**を出す。**操作も口も経路も増えない**
——行が持つのは `cleared` の行と同じ「審査待ちへ戻す」だけで、`ADMIN_OPEN_ROUTES` にも
1 行も足していない（権限が無ければ `/` は 404 のまま）。

**条件は `src/reports.ts` の `REVIEW_RENAMED_SQL` 1 か所にある。** 画面はそれを import し、
...
**忘れるとどうなるか。** 改名の節と「問題なしとした作品」の節が `title_changes` を読むので、
その 2 節が「no such table」で落ちる。**画面ごとは落とさない**——**審査待ちの節は出る**。
落ちた 2 節は**件数の代わりに「読み込めませんでした」**と出し（0 件と区別する）、
**画面のステータスは 500** になる（成功したかのようにログへ残さない。
`src/admin/review.ts` の `readSection`）。
...
**画面の「審査待ち」と「改名されて通報が付いた作品」の 2 節に並ぶ id と、同じ集合になる**
こと。**ずれてよいのは次の 2 つだけである**（どちらも #367 より前からの差）。
...
### 既知の穴: もう一度「問題なし」にしても、この節から消えない

**`renameGame` は `cleared` を `NULL` へ戻す**ので、改名のあとの通報は**まず `queued` へ
入る**（閾値は 1 人）。運営がそれを見て「問題なし」にすると、**改名以降の通報が残って
いるので条件に当たり、改名の節へ戻ってくる**——次に作者が改名するまで出続ける
（手元で実測した。`recordReport` → `setReviewState` → `renameGame` → `recordReport` →
`setReviewState` の順で、最後の 1 手で当たり始める）。

- **画面の注記にこの挙動を書いてある**（運営が「消えない」を不具合と読まないため）
- **#367 では塞いでいない。** 直すなら条件そのもの（`src/reports.ts`。たとえば
  「最後に `cleared` にした時刻より後の通報」に絞る）であり、**スクリプトと画面が同時に
  直る**。`scripts/report-queue.sh` も同じ行を出し続けている
- 「見たので一覧から外す」操作を画面に足す形は採らない（#367 の scope.out が
  新しい操作を禁じている）
