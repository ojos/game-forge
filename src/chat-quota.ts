 ...
 * **理由は 1 往復の額が 1 桁小さいこと**（見込み 約 2.2 円 対 実測 ¥22.41）である。
 *
 * ## 往復数ではなくトークン数で数える
 *
 * **ソースを渡すかどうかで 1 往復の重さが 5 倍以上変わる**（利用者の決定。5.16）。ソースを渡さない
 * 1 往復は約 3,200 トークンだが、64 KiB のソースを 1 回渡すと約 19,200 トークンになる。
