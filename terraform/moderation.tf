...
locals {
  /**
   * コンテンツフィルタの強度（8.2）。**実測で決め直した値である。**
   *
   * ## VIOLENCE と MISCONDUCT を NONE にした理由（2026-09-03 の実測）
   *
   * **`LOW` でも 2D ゲームの主流ジャンルが落ちた。** STANDARD tier で日本語が見えるように
   * なった直後に、ありそうなお題 8 件を本物の Guardrail へ当てた結果である。
   *
   * | お題 | 落ちたフィルタ |
   * |---|---|
   * | 10 秒で地球を破壊するゲーム | **MISCONDUCT**（**仕様書 11.3 が M7-2 のサンプルに名指しした題材**） |
   * | 銀行から金塊を盗み出す脱出ゲーム | MISCONDUCT |
   * | 刑務所から脱獄するアクション | VIOLENCE（**既に LOW**）＋ MISCONDUCT |
   * | 戦車で敵基地を破壊する | VIOLENCE（**既に LOW**）＋ MISCONDUCT |
   * | ゴリラがソーダを振る / 単位を落とすな留学生 / 隕石から街を守る / 毒キノコを避ける | 通る |
   *
   * **`LOW` は「最弱」ではなく「最弱の一段上」である**（NONE < LOW < MEDIUM < HIGH）。
   * 脱獄と戦車は `LOW` でも落ちたので、通すには `NONE` しかない。
   *
   * **MISCONDUCT はゲームの前提と正面からぶつかる。** 盗む・逃げる・壊すは題材であって
   * 行動計画ではない。**この 2 つを外しても、本当に危険な入力は HATE / INSULTS /
   * SEXUAL / PROMPT_ATTACK が拾う**（同じ実測で、日本語の暴力的な脅迫文は VIOLENCE と
   * MISCONDUCT の両方で止まっていた——**つまりこの 2 つを外すと、その文は素通りする**）。
   *
   * ## 外したことの代償を書いておく
   *
   * **暴力表現の止め役は、8.3 の NG ワード検査（`src/denied-terms.ts`）と 8.4 の通報
   * （M6-4）だけになる。** 8.3 自身が「安い検査で大半を捕まえる層」と位置づけられており、
   * **8.2 が暴力を見ていないことを前提に読み直す必要がある。** これは値の調整ではなく、
   * **防御の層が 1 枚減ったという事実**である。
   *
   * ## プロンプト攻撃だけ高い
   *
   * この LLM はシステムプロンプトで import 制限（6.1）と命名規制（6.2）を掛けており、
   * **そこを外されると検査の前提が崩れる。**
   *
   ...
   */
  moderation_filters = {
    # **NONE は「宣言していない」ではなく「見ないと決めた」である。** 項目ごと消すと、
    # 次に読む人が「足し忘れ」と読む。決定として残す。
    VIOLENCE      = "NONE"
    MISCONDUCT    = "NONE"
    HATE          = "MEDIUM"
    INSULTS       = "MEDIUM"
