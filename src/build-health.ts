 ...
 * 最初に締める場所である。**
 *
 * **3 以上にしない。** 4.3 の逆算どおり、サービス全体の生成は 1 日あたり約 34 回、
 * すなわち **1 時間に 1.4 回**である。窓を {@link BUILD_STOP_WINDOW_SECONDS}（15 分）に
 * 取ると、その窓に入る依頼はそもそも平均 0.35 件しかない。3 件を求める信号は
 * **障害が起きても発火しない**——「誤爆しない」ではなく「何も検知しない」であり、
 * 見逃しの側へ倒し切ることになる。
 *
