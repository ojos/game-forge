 ...
 * D1 も R2 も引かない。
 *
 * **行を作るのはソースを読み切ったあとである。** 先に作ると、上限超や R2 の取りこぼしで
 ...
 *
 * 5.3 は上限超過時に「作者の選択で LLM に整理させる」と定め、その 1 つ目の条件として
 * **入力段階での事前警告**（親ソースが **52,428 バイト**＝上限 64KB の 80% を超えていたら
 * 伝える）を課す。
 ...
 * 「やめる」を選んでも跡が残らない。
 *
 * ## 上限超は整理を問う（確定18 の条件 2〜4）
 *
 * ~~## 30KB 超はいまも断るしかない（確定18 の条件 2〜4 は未実装である）~~
 * ~~5.3 の整理パスは**この経路からは動かせない。**~~
 * **→ #33 / #258（M5-2）で動くようになった。この節の「未実装」は覆っている。**
 * いまは {@link decideForkSizeAction} が `offer-tidy` を返し、作者が選べば `tidy` として
 * 整理パスへ入る（下記の `handleFork`）。**#284 の掃除で見つけた食い違いで、
 * #284 が作ったものではない。**
 *
 * ふさがっていた理由は 2 つとも構造的なもので、**なぜ版 3 が要ったのかの説明として
 * 残す。**
 *
 * 1. **エッジは Bedrock を呼べない**（`BEDROCK_AWS_*` は #160 で削除された）。整理は
 *    全文再出力の LLM 呼び出しなので、オーケストレータ Lambda の中でしか走らない。
 *    **これはいまも変わらない**——だから整理も Lambda 側で走る。
 * 2. **`src/orchestrator/payload.ts` が上限超の `baseSource` を断る。** 整理の入力は
 *    まさに上限超のソースであり、**上限を守るための検査が逃げ道の入口をふさいでいた。**
 *    **→ 版 3 を名乗る本文だけが `TIDY_MAX_SOURCE_BYTES` まで載せられる形で開けた**
 *    （#258。版 1・版 2 の受け入れ条件は 1 文字も変えていない）。
 *
 * 詳しくは `src/source-size.ts` の冒頭にある。**整理しても収まる見込みが無い大きさ
 * （上限の 2 倍超）は、問わずに断る。断るときも「もう一度」と言わない**
 ...
 *
 * **公開済みの作品だけを返す**（5.3）。**この判定を書き写さない**——#727 のチャットが
 * 同じ条件を自前で書いたところ、**「フォークの条件を変えた日に、チャットだけが古い条件で通る」**
 * という形になった（Copilot の指摘）。**引く側が増えるたびにここを通す。**
 *
 * **題名・説明・タグも一緒に返す**のは、チャットの文脈（確定38）がそれを読むためである。
...
export async function readParentSource(
  ...
  // **整理パスの上限まで読む**（確定18 の条件 2 / #33）。`MAX_SOURCE_BYTES` で断ってしまうと、
